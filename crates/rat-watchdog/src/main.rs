// ═══════════════════════════════════════════════════════════════════════════════
// rat-watchdog — Hardware Kill Switch & Heartbeat Monitor
//
...
impl WatchdogState {
    fn new(shutdown_tx: watch::Sender<bool>) -> Self {
        Self {
            ...
            alert_url: std::env::var("RAT_ALERT_WEBHOOK_URL").ok(),
...
async fn trigger_emergency_halt(state: &WatchdogState) {
    ...
    error!("══════════════════════════════════════════════════════════");
    error!("  🚨 RAT WATCHDOG EMERGENCY HALT TRIGGERED");
    error!("══════════════════════════════════════════════════════════");
    ...
    send_alert(
        state,
        "🚨 RAT WATCHDOG: Emergency HALT triggered — orchestrator heartbeat lost.",
    )
    .await;
...
async fn send_alert(state: &WatchdogState, message: &str) {
    if let Some(ref url) = state.alert_url {
        ...
        warn!("[Watchdog] No RAT_ALERT_WEBHOOK_URL configured — alert not sent.");
    }
}
...
async fn halt_handler(State(state): State<WatchdogState>) -> (StatusCode, Json<serde_json::Value>) {
    ...
    send_alert(
        &state,
        "🛑 RAT WATCHDOG: Manual HALT triggered via HTTP endpoint.",
    )
    .await;
...
async fn main() {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "rat_watchdog=info".into()),
        )
        .init();
    ...
    if std::env::var("RAT_ALERT_WEBHOOK_URL").is_ok() {
        info!("[Watchdog] 📲 Alert webhook configured.");
    } else {
        warn!("[Watchdog] ⚠ No alert webhook configured. Set RAT_ALERT_WEBHOOK_URL.");
    }
