...
use cotrader_autonomous::orchestrator_struct::AutonomousOrchestrator;
use cotrader_core::paper_engine::{BrokerRegistry, OrderRequest, OrderType};
...
impl RuntimeEngine {
    ...
    /// Update or create 1m OHLCV bar using cotrader_core::OhlcvBar (which uses String timestamps).
    ...
    fn update_ohlcv_core(
        history: &mut Vec<cotrader_core::OhlcvBar>,
        price: f64,
        now_rfc: &str,
        tick_count: &mut u32,
    ) {
        *tick_count += 1;
        ...
        if history.is_empty() {
            history.push(cotrader_core::OhlcvBar {
    ...
    async fn run_backtest(
        ...
        while feed.has_next() {
            ...
            for bar in bars {
                ...
                {
                    let ohlcv_bar = cotrader_core::OhlcvBar {
                    ...
                    for sym in &self.symbols {
                        if let Some(bars) = history.get(sym) {
                            if bars.len() >= 10 {
                                ...
                                let regime = cotrader_autonomous::helpers::estimate_market_regime(
                                    &prices, &highs, &lows,
                                );
    ...
    async fn run_validate(
                            ...
                            for sym in &self.symbols {
                                if let Some(bars) = history.get(sym) {
                                    if bars.len() >= 10 {
                                        ...
                                        let lows: Vec<f64> = bars.iter().map(|b| b.low).collect();
                                        let regime =
                                            cotrader_autonomous::helpers::estimate_market_regime(
                                                &prices, &highs, &lows,
                                            );
        ...
        use cotrader_autonomous::execution_coordinator::init_outcome_processor;
        ...
        let mut orch_clone = cotrader_autonomous::orchestrator_struct::AutonomousOrchestrator::new(
            self.orchestrator.state.clone(),
        );
        orch_clone.init_rat(); // required so run_full_pipeline doesn't panic
    ...
    pub async fn make_decision(
        ...
        symbol: &str,
        current_price: f64,
    ) -> (Option<cotrader_autonomous::types::TradeSignal>, String, bool) {
        ...
        if let Some(entry) = cached {
            ...
            if let Some(signal) = self
                ...
                .await
            {
                // Portfolio composition check before execution
                let direction = if signal.direction == cotrader_core::TradeDirection::Long {
                    cotrader_core::TradeDirection::Long
                } else {
                    cotrader_core::TradeDirection::Short
                };
                match self.portfolio_reasoner.should_open_new(&signal.symbol, direction).await {
                    crate::portfolio_reasoner::Decision::Reject { reason } => {
                        tracing::info!("PortfolioReasoner REJECT for {}: {}", symbol, reason);
                        return (Some(signal), format!("cache:portfolio_reject:{}", reason), false);
                    }
                    crate::portfolio_reasoner::Decision::Caution { reason } => {
                        tracing::warn!("PortfolioReasoner CAUTION for {}: {}", symbol, reason);
                    }
                    crate::portfolio_reasoner::Decision::Approve { .. } => {}
                }

                // Active learner: check if exploration sizing should override
                if let Some(explore_size) = self.active_learner.maybe_explore(&signal.symbol, direction).await {
                    tracing::info!("ActiveLearner: exploration probe size={:.4} for {}", explore_size, symbol);
                    // Exploration sizing is advisory — logged but doesn't override cache signal
                }
    ...
    async fn execute_live_trade(
        &self,
        signal: &cotrader_autonomous::types::TradeSignal,
        market_price: f64,
    ) -> Result<(), String> {
        let registry = self
            .broker_registry
    ...
    async fn signal_from_cache(
        ...
        current_price: f64,
    ) -> Option<cotrader_autonomous::types::TradeSignal> {
        ...
        Some(cotrader_autonomous::types::TradeSignal {
