...
pub struct RiskManager {
    ...
    /// FIXED (fault-isolation audit, D3): true 60-second sliding window for
    /// rapid-fire protection. The previous AtomicU64 never decayed — after one
    /// burst it rejected every trade until an outcome was recorded, and the
    /// hard stop never engaged on a velocity burst.
    rapid_orders: parking_lot::Mutex<std::collections::VecDeque<Instant>>,
}

/// Velocity limit: more than this many order attempts inside `RAPID_WINDOW`
/// engages the hard stop.
const RAPID_MAX_IN_WINDOW: usize = 10;
...
impl RiskManager {
    pub fn new() -> Self {
        ...
        Self {
            ...
            rapid_orders: parking_lot::Mutex::new(std::collections::VecDeque::new()),
        }
    }
    ...
    pub async fn check_trade(
        ...
        // Global coordination (D4 fix): a halt announced by any other
        // subsystem (trading breaker, memory breaker) blocks trades here too.
        if cotrader_core::breaker_net::is_halted_by_other(cotrader_core::breaker_net::BreakerId::Risk) {
            return RiskDecision::Rejected {
                reason: RiskRejection::CircuitBreakerTriggered {
                    reason: "Global halt announced by another subsystem".into(),
                },
            };
        }

        // Rapid-fire protection — true sliding window (D3 fix). A burst beyond
        // the limit engages the internal hard stop and announces it globally.
        {
            let now = Instant::now();
            let mut w = self.rapid_orders.lock();
            while w.front().is_some_and(|t| now.duration_since(*t) > RAPID_WINDOW) {
                w.pop_front();
            }
            w.push_back(now);
            if w.len() > RAPID_MAX_IN_WINDOW {
                drop(w);
                self.set_hard_stop(true);
                return RiskDecision::Rejected {
                    reason: RiskRejection::CircuitBreakerTriggered {
                        reason: format!(
                            "Trade velocity burst: >{} orders in {}s — hard stop engaged",
                            RAPID_MAX_IN_WINDOW,
                            RAPID_WINDOW.as_secs()
                        ),
                    },
                };
            }
    ...
    pub fn record_trade_outcome(&self, pnl: f64) {
        ...
        // NOTE (D3 fix): the rapid-order window decays with time on its own;
        // recorded outcomes no longer mass-reset velocity protection.
    }
    ...
    pub fn set_hard_stop(&self, engaged: bool) {
        ...
        if engaged {
            ...
            // Propagate to the global breaker coordination channel (D4 fix):
            cotrader_core::breaker_net::announce_halt(
                cotrader_core::breaker_net::BreakerId::Risk,
                "RiskManager hard stop engaged",
            );
        } else {
            tracing::info!("Hard stop released — trading resumed");
            cotrader_core::breaker_net::resume(cotrader_core::breaker_net::BreakerId::Risk);
        }
    }
