...
use cotrader_core::{Config, DisciplineRules, MemoryStore};
use cotrader_core::paper_engine::BrokerAdapter;
use cotrader_runtime::engine::RuntimeEngine;
use cotrader_runtime::event_bus::{AgentEvent, EventBus};
...
fn setup_test_env(test_name: &str) -> (AutonomousOrchestrator, Vec<String>, String) {
    ...
    let state = cotrader_autonomous::state::SharedState::new(memory, rules, config, &sqlite_path, paper_broker)
...
async fn test_eventbus_publish_subscribe() {
    ...
    bus.publish(AgentEvent::PriceTick {
        ...
        source: cotrader_runtime::event_bus::PriceSource::Rest,
    });
...
async fn test_runtime_engine_construction() {
    ...
    engine.event_bus().publish(AgentEvent::PriceTick {
        ...
        volume: 50.0,
        timestamp: chrono::Utc::now(),
        source: cotrader_runtime::event_bus::PriceSource::Manual,
    });
...
async fn test_api_client_fetch_price() {
    ...
    match cotrader_runtime::api_clients::fetch_live_bar(&client, "BTC").await {
        Ok(bar) => {
