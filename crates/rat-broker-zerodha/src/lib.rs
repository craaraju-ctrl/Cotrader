...
impl ZerodhaKiteBroker {
    ...
    pub fn new(api_key: &str, api_secret: &str, base_url: &str, request_token: &str) -> Self {
        let client = reqwest::Client::builder()
            .user_agent("rat/0.2.0 (Kite Connect v3)")
    ...
    /// Convert a Kite position list to RAT Position list.
    fn kite_positions_to_rat(
        ...
        for pos in positions {
            ...
            result.push(Position {
                ...
                unrealized_pnl: pnl,
                unrealized_pnl_pct,
                status: rat_core::paper_engine::PositionStatus::Open,
                opened_at: now,
                closed_at: None,
    ...
    /// Map RAT OrderType to Kite order type string.
    ...
    /// Map RAT TradeDirection to Kite transaction type.
    ...
    /// Map Kite status to RAT OrderStatus.
...
impl BrokerAdapter for ZerodhaKiteBroker {
    ...
    async fn get_positions(&self) -> Result<Vec<Position>, String> {
        ...
        Ok(Self::kite_positions_to_rat(&positions, &holdings))
    }
        ...
        let mut trades: Vec<ClosedTrade> = orders
            ...
            .map(|o| {
                ...
                ClosedTrade {
                    ...
                    close_reason: rat_core::paper_engine::CloseReason::Manual,
                    opened_at: now,
                    closed_at: now,
                    duration_secs: 0,
                    strategy: Some("Zerodha Live".to_string()),
    ...
    async fn update_price(
        ...
        // Kite doesn't auto-close on SL/TP — that's handled by RAT's own risk engine.
