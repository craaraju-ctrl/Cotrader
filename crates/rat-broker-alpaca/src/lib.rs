...
impl AlpacaBroker {
    ...
    pub fn new(api_key_id: &str, api_secret_key: &str, paper: bool) -> Self {
        let client = reqwest::Client::builder()
            .user_agent("rat/0.2.0 (Alpaca API v2)")
    ...
    /// Map an Alpaca position to a RAT Position.
    fn alpaca_position_to_rat(pos: &AlpacaPosition) -> Position {
    ...
    /// Map Alpaca order status to RAT OrderStatus.
...
impl BrokerAdapter for AlpacaBroker {
    ...
    async fn place_order(
        ...
        if let Some(ref tag) = request.strategy {
            ...
            body["client_order_id"] = serde_json::json!(format!("rat-{}", tag));
        }
    ...
    async fn get_positions(&self) -> Result<Vec<Position>, String> {
        ...
        Ok(positions
            .iter()
            .map(Self::alpaca_position_to_rat)
            .collect())
    }
    ...
    async fn update_price(
        ...
        // RAT's own risk engine monitors positions separately.
        let _ = symbol;
        Ok(Vec::new())
    }
