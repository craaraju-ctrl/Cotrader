...
use cotrader_core::paper_engine::TradingMode;
// TODO: Implement cotrader-eventbus crate
// TODO: Implement cotrader-eventbus crate
// use cotrader_eventbus::{self, subjects as event_subjects, EventBus, RatEvent};

// ── Loop Manager to dynamically start and stop the background temporal loops ──
struct LoopManager {
    orchestrator: cotrader_autonomous::AutonomousOrchestrator,
...
async fn restore_portfolio_state(state: &cotrader_autonomous::state::SharedState) -> bool {
    match state.agent_memory.memory.load_state("portfolio/state") {
        Ok(Some(json)) => {
            match serde_json::from_str::<cotrader_autonomous::types::PortfolioState>(&json) {
                Ok(restored) => {
...
async fn restore_agent_tasks(state: &cotrader_autonomous::state::SharedState) {
    if let Ok(Some(json)) = state.agent_memory.memory.load_state("tasks/state") {
        if let Ok(restored) = serde_json::from_str::<Vec<cotrader_autonomous::state::AgentTask>>(&json)
        {
...
async fn graceful_shutdown(orchestrator: &cotrader_autonomous::AutonomousOrchestrator) {
...
async fn portfolio_snapshot_response(state: &WebState, include_meta: bool) -> serde_json::Value {
    ...
    let mut snapshot = cotrader_autonomous::state::SharedState::portfolio_snapshot_json(&portfolio);
    if include_meta {
...
async fn save_watchlist(state: &cotrader_autonomous::state::SharedState) {
...
async fn restore_watchlist(state: &cotrader_autonomous::state::SharedState) {
...
async fn add_to_watchlist(
    ...
    let scanner =
        cotrader_autonomous::scanner::WatchlistScannerAgent::new(state.orchestrator.state.clone());
...
async fn execute_trade(
    ...
    use cotrader_autonomous::types::TradeSignal;
    use cotrader_core::{validate_trade_setup, TradeDirection, TradeSetup};
    ...
    let context = cotrader_core::MarketContext {
    ...
    let position_size = {
        ...
        cotrader_autonomous::helpers::calculate_position_size_with_cash(
            portfolio_equity,
...
async fn trigger_orchestra_cycle(
    ...
    let sym = req
        .symbol
        .map(|s| cotrader_core::normalize_base_symbol(&s))
        .filter(|s| !s.is_empty())
        .unwrap_or_else(|| {
    ...
    let outcome = cotrader_autonomous::run_single(&state.orchestrator, &client, &sym).await;
...
async fn run_pipeline_batch(
    ...
    let symbols: Vec<String> = if let Some(list) = req.symbols {
        ...
        vec![cotrader_core::normalize_base_symbol(&sym)]
    } else {
    ...
    let batch = cotrader_autonomous::run_batch(&state.orchestrator, &client, &symbols).await;
...
/// Build backtest contexts from REAL OHLCV history (no synthetic data).
/// Picks the symbol with the deepest history. Returns Err if no real data is
/// loaded yet — a backtest on fabricated bars produces fabricated statistics,
/// which is worse than no backtest.
async fn real_backtest_contexts(
    state: &WebState,
) -> Result<(String, Vec<cotrader_core::MarketContext>), String> {
    let hist = state.orchestrator.state.market_data.ohlcv_history.read().await;
    let (sym, bars) = hist
        .iter()
        .max_by_key(|(_, b)| b.len())
        .ok_or_else(|| "No OHLCV history loaded yet — start the pipeline first".to_string())?;
    if bars.len() < 10 {
        return Err(format!(
            "Only {} bars of real history for {} — need at least 10",
            bars.len(),
            sym
        ));
    }
    let mut ctxs = Vec::with_capacity(bars.len() - 1);
    for i in 1..bars.len() {
        let ts = chrono::DateTime::parse_from_rfc3339(&bars[i].timestamp)
            .map(|d| d.with_timezone(&chrono::Utc))
            .unwrap_or_else(|_| chrono::Utc::now());
        ctxs.push(cotrader_core::MarketContext {
            symbol: sym.clone(),
...
async fn run_backtest(State(state): State<WebState>) -> impl axum::response::IntoResponse {
    // FIXED (real-code audit): this endpoint backtested 50 SYNTHETIC
    // monotonically-rising NIFTY bars and logged the fake stats into the CoT.
    // It now replays the real Binance OHLCV history already in memory.
    let (symbol, real_data) = match real_backtest_contexts(&state).await {
        Ok(v) => v,
        Err(e) => {
            return Json(serde_json::json!({ "error": e }));
        }
    };
    let bars_used = real_data.len();
    let rules = state.orchestrator.state.rule_engine.rules.read().await;
    let mut backtester = cotrader_core::Backtester::new(rules.clone());
    let result = backtester.run_simulation(real_data);
    let backtest_desc = format!(
        "Replaying {} real {} bars through the rule engine",
        bars_used, symbol
    );
...
/// FIXED (real-code audit): was 50 synthetic NIFTY bars; now replays real history.
async fn get_backtest_results(State(state): State<WebState>) -> impl axum::response::IntoResponse {
    ...
    Json(serde_json::json!({
        ...
        "message": format!(
            "{} on {} real bars | Trades: {} | Win Rate: {:.1}% | Total P&L: ${:.2} | Max DD: {:.2}% | Sharpe: {:.2}",
            symbol, bars_used,
            result.total_trades, result.win_rate * 100.0,
...
async fn get_agent_tree() -> impl axum::response::IntoResponse {
    Json(cotrader_autonomous::Rat::tree_json())
}
...
async fn fetch_live_stock_price(
    ...
    // FIXED (real-code audit): a Yahoo failure used to return a hardcoded
    // 24500.0 that looked like a real price. 0.0 is the honest "unknown" —
    // every consumer already treats non-positive prices as invalid.
...
async fn get_market_depth(
    ...
    // For stocks (or if Binance fails), synthesize depth around the symbol's
    // REAL last close from OHLCV history (was: DUMMY_PRICE env / hardcoded
    // 2950). If no real price exists, return an explicit error instead of a
    // fabricated book — fake data must never look real.
    let real_last_close = {
        let hist = state.orchestrator.state.market_data.ohlcv_history.read().await;
        hist.get(&sym).and_then(|bars| bars.last().map(|b| b.close))
    };
    let price = match real_last_close {
        Some(p) if p > 0.0 => p,
        _ => {
            return Json(serde_json::json!({
                "symbol": sym,
                "error": "No real price available for synthetic depth — no OHLCV history for this symbol",
                "bids": [], "asks": [],
                "source": "none"
            }));
        }
...
async fn get_crypto_prices(
    ...
    let symbols: Vec<String> = req
        ...
        .map(str::trim)
        .filter(|s| !s.is_empty())
        .map(cotrader_core::normalize_base_symbol)
        .collect();
...
async fn get_all_prices(State(state): State<WebState>) -> impl axum::response::IntoResponse {
    ...
    let crypto_syms: Vec<&str> = symbols
        .iter()
        .filter(|s| cotrader_core::is_crypto_symbol(s))
        .map(String::as_str)
        .collect();
...
async fn get_policy_cache(State(state): State<WebState>) -> impl axum::response::IntoResponse {
    let cache =
        cotrader_runtime::policy_cache::PolicyCache::from_disk(state.orchestrator.state.clone());
...
async fn get_news(State(state): State<WebState>) -> impl axum::response::IntoResponse {
    let client = reqwest::Client::new();
    let fetcher = cotrader_core::NewsFetcher::new(client, (*state.orchestrator.state.io.config).clone()); // free news APIs + keys (research: Alpha Vantage, Finnhub etc.)
...
async fn main() {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env().unwrap_or_else(|_| {
                "cotrader_orchestrator=info,cotrader_autonomous=info,cotrader_core=info".into()
            }),
        )
        .init();
    ...
    println!("╚══════════════════════════════════════════════════════╝");
    cotrader_autonomous::Rat::print_tree();

    // Create paper broker for the trading system
    let paper_broker: std::sync::Arc<dyn cotrader_core::paper_engine::BrokerAdapter> = 
        std::sync::Arc::new(cotrader_runtime::broker::plugin_registry::PaperBroker::new());

    let mut orchestrator = match initialize_autonomous_system(paper_broker).await {
        Ok(o) => o,
        Err(e) => {
    ...
    // === NEW: Initialize the global OutcomeProcessor for self-evolution ===
    {
        ...
        cotrader_autonomous::execution_coordinator::init_outcome_processor(
    ...
    // TODO: Implement cotrader-eventbus crate for pub-sub communication
    // This bridge forwards pipeline events from EventBus to WebSocket for TUI display
    // {
    ...
    //             return;
    //         };
    //         let mut signal_stream = match bus.subscribe(&event_subjects::all_signal_events()).await {
    //             Ok(stream) => stream,
    //             Err(e) => {
    ...
    cotrader_autonomous::orchestrator_pipeline::start_metrics_health_check_loop();

    // ── ML Training Loop ──────────────────────────────────────────────
    // Background ML model training. Retrains win probability and strategy
                    ...
                    match broker.get_order_status(&order.broker_order_id).await {
                        Ok(status) => {
                            let is_terminal = matches!(
                                status,
                                cotrader_core::paper_engine::OrderStatus::Filled
                                    | cotrader_core::paper_engine::OrderStatus::Cancelled
                                    | cotrader_core::paper_engine::OrderStatus::Expired
                            ) || matches!(
                                status,
                                cotrader_core::paper_engine::OrderStatus::Rejected { .. }
                            );
    ...
    // Spawn ReconciliationEngine loop: runs every 60s in live mode
    {
        ...
        tokio::spawn(async move {
            loop {
                ...
                let engine = cotrader_autonomous::reconciliation_engine::ReconciliationEngine::new(
    ...
    // ── Zerodha Kite ──────────────────────────────────────────────────────
    // TODO: Add cotrader-broker-zerodha to orchestrator dependencies
    // {
    ...
    //         let live_broker = cotrader_broker_zerodha::create_zerodha_broker(api_key, api_secret, request_token);
    //         orchestrator.state.portfolio_store.broker_registry.register_live_broker(live_broker).await;
    ...
    // ── Upstox ────────────────────────────────────────────────────────────
    // TODO: Implement cotrader-broker-upstox crate
    // {
    ...
    //         let live_broker = cotrader_broker_upstox::create_upstox_broker(cid, cs, &redirect_uri, token);
    //         orchestrator.state.portfolio_store.broker_registry.register_live_broker(live_broker).await;
    ...
    // ── Angel One ─────────────────────────────────────────────────────────
    // TODO: Implement cotrader-broker-angelone crate
    // {
    ...
    //         let live_broker = cotrader_broker_angelone::create_angelone_broker(ak, ci, p, totp_secret);
    //         orchestrator.state.portfolio_store.broker_registry.register_live_broker(live_broker).await;
    ...
    // ── 5Paisa ────────────────────────────────────────────────────────────
    // TODO: Implement cotrader-broker-5paisa crate
    // {
    ...
    //     if let (Some(ak), Some(ek), Some(ui), Some(cc)) = (&app_key, &encry_key, &user_id, &client_code) {
    //         let live_broker = cotrader_broker_5paisa::create_fivepaisa_broker(ak, ek, ui, cc);
    //         orchestrator.state.portfolio_store.broker_registry.register_live_broker(live_broker).await;
    ...
    // TODO: Implement cotrader-broker-alpaca crate
    // {
    ...
    //         let live_broker = cotrader_broker_alpaca::create_alpaca_broker(api_key, api_secret, paper);
    //         orchestrator.state.portfolio_store.broker_registry.register_live_broker(live_broker).await;
    ...
    // ── CoTrader (Universal Broker) ────────────────────────────────────────
    // TODO: Implement cotrader-broker-cotrader crate for exchange connectivity
    // {
    //     let co_base_url = std::env::var("COTRADER_BASE_URL").unwrap_or_else(|_| "http://localhost:8080".to_string());
    //     let co_api_key = std::env::var("COTRADER_API_KEY").unwrap_or_default();
    //     let co_secret = std::env::var("COTRADER_SECRET_KEY").unwrap_or_default();
    //     let co_user_id = std::env::var("COTRADER_USER_ID").unwrap_or_else(|_| "orchestra".to_string());
    //     let live_broker: std::sync::Arc<dyn cotrader_core::paper_engine::BrokerAdapter> =
    //         std::sync::Arc::new(cotrader_broker_cotrader::CoTraderBroker::new(
    //             &co_base_url, &co_api_key, &co_secret, &co_user_id
    //         ));
    //     orchestrator.state.portfolio_store.broker_registry.register_live_broker(live_broker).await;
    //     info!("CoTrader exchange broker registered (base: {})", co_base_url);
    // }
    ...
    //     cotrader_eventbus::create_event_bus()
    //         .await
    ...
    //             &RatEvent::SystemControl(cotrader_eventbus::SystemControlEvent {
