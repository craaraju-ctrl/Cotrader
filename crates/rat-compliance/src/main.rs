// ═══════════════════════════════════════════════════════════════════════════════
// rat-compliance — Pre-Trade Compliance Gateway
//
...
//   - Rules configured via TOML file (~/.rat/compliance.toml) — read-only at startup
...
//   Gateway       ──(append SQLite)──→ ~/.rat/compliance.db
//
...
const DEFAULT_CONFIG_PATH: &str = "~/.rat/compliance.toml";
const DB_PATH: &str = "~/.rat/compliance.db";
...
fn write_default_config(path: &str) {
    ...
    let content = format!(
        "# rat-compliance configuration file\n\
...
async fn main() {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "rat_compliance=info".into()),
        )
        .init();
    ...
    let app = Router::new()
        ...
        .route("/health", get(|| async { Json(serde_json::json!({"status": "ok", "service": "rat-compliance", "version": env!("CARGO_PKG_VERSION")})) as Json<serde_json::Value> }))
