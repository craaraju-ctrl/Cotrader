...
use rat_core::episode::{MarketStateSnapshot, ReasoningStep, TradingEpisode};
use rat_core::{
    calculate_confluence_score, calculate_pivot_points, Agent, MarketContext, OhlcvBar, PivotMethod,
};
use rat_eventbus::{subjects as event_subjects, EventBus, RatEvent};
...
pub async fn fast_loop(
    ...
    loop {
            ...
            let handle = tokio::spawn(async move {
                ...
                let _ = bus_clone
                    .publish(
                        ...
                        &RatEvent::MarketPrice(rat_eventbus::MarketPriceEvent {
                            symbol: sym.clone(),
                            price,
        ...
        if cycle_num % 60 < 6 {
            ...
            let port_snapshot =
                RatEvent::PortfolioSnapshot(rat_eventbus::PortfolioSnapshotEvent {
            ...
            let health = RatEvent::Health(rat_eventbus::HealthEvent {
    ...
    loop {
        ...
        for symbol in assets.clone() {
            ...
            let handle = tokio::spawn(async move {
                ...
                if price > 0.0 {
                    let meter =
                        rat_autonomous::market_metrics_meter::MarketMetricsMeter::new(st.clone());
                    ...
                    let inferred_regime = match snap.regime_hint.as_str() {
                        ...
                        "volatile" => Some(rat_autonomous::types::MarketRegime::Volatile),
                        _ => None,
                    };
        ...
        for symbol in assets.clone() {
            ...
            ohlcv_handles.push(tokio::spawn(async move {
                ...
                let bars = if is_crypto {
                    rat_core::fetch_klines(&cl, &sym, "1m", 100).await.ok()?
                } else {
    ...
    loop {
        tokio::select! {
            ...
            _ = sleep(Duration::from_secs(slow_secs)) => {
                ...
                // 2. Run meta-control
                let meta = rat_autonomous::meta_control::MetaControlAgent::new(state.clone());
                    ...
                    if avg_regret > 0.65 {
                        ...
                        let current_regime = {
                            ...
                            match *regime {
                                ...
                                Some(rat_autonomous::types::MarketRegime::TrendingBear) =>
                                    rat_autonomous::regime_classifier::MarketRegime::TrendingBear,
                                Some(rat_autonomous::types::MarketRegime::Ranging) =>
                                    rat_autonomous::regime_classifier::MarketRegime::Ranging,
                                Some(rat_autonomous::types::MarketRegime::Volatile) =>
                                    rat_autonomous::regime_classifier::MarketRegime::Volatile,
                                _ => rat_autonomous::regime_classifier::MarketRegime::Ranging,
                            }
                        };
...
async fn capture_trade_episode(
    orchestrator: &AutonomousOrchestrator,
    summary: &rat_autonomous::types::PipelineSummary,
) {
    if let Some(ref signal) = summary.final_signal {
        let now = Utc::now();
        ...
        let regime_str = match *regime {
            ...
            Some(rat_autonomous::types::MarketRegime::Ranging) => "Ranging",
            Some(rat_autonomous::types::MarketRegime::Volatile) => "Volatile",
            Some(rat_autonomous::types::MarketRegime::LowLiquidity) => "LowLiquidity",
            None => "Unknown",
        };
        drop(regime);
...
use rat_autonomous::state::TimeframeData;
use rat_autonomous::types::PortfolioState;

pub fn is_crypto_symbol(symbol: &str) -> bool {
    rat_core::is_crypto_symbol(symbol)
}
...
async fn fetch_price(
    ...
    if is_crypto {
        match rat_core::fetch_binance_price(client, symbol).await {
            Ok(p) => Ok(p),
            Err(e) => {
...
pub async fn fetch_binance_price(
    ...
    rat_core::fetch_binance_price(client, symbol).await
}
...
pub async fn fetch_binance_24h_ticker(
    ...
    rat_core::fetch_ticker_24hr_raw(client, symbol).await
}
...
pub async fn fetch_binance_klines(
    ...
    rat_core::fetch_klines(client, symbol, interval, limit).await
}
...
async fn compute_mtf_analysis(symbol: &str, state: &SharedState) {
    ...
    for tf_data in &tf_data_map {
        ...
        let metrics = rat_autonomous::market_metrics_meter::MarketMetricsMeter::compute_on_bars(
            bars,
            symbol,
            current_price,
            tf_label,
        );
        let patterns = rat_core::detect_patterns(bars);
        ...
        let analysis = rat_autonomous::state::TimeframeAnalysis {
    ...
    let aggregate = rat_autonomous::state::MultiTfAggregate {
        symbol: symbol.to_string(),
...
async fn log_portfolio_snapshot_full(orchestrator: &AutonomousOrchestrator) {
    ...
    for pos in &p.open_positions {
        let dir = if pos.direction == rat_core::TradeDirection::Long {
            "LONG"
        } else {
            "SHORT"
        };
