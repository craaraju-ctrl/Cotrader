...
//
// Run: cargo test -p rat-autonomous --test no_kronos_no_llm_pipeline
...
async fn test_identifier_with_kronos_down() {
    ...
    let result = orch.rat().run_identifier("ETH", 3_500.0, 1).await;
...
async fn test_debate_agents_without_llm() {
    ...
    let input = rat_core::AgentInput::ConfluenceRequest {
        context: rat_core::MarketContext {
    ...
    let (action, conf, reason, turns) = rat_autonomous::debate::run_debate(
        orch.state.clone(),
        &input,
...
async fn test_all_16_agents_execute() {
    ...
    let ident_result = orch.rat().run_identifier("BTC", 65_000.0, 1).await;
    assert!(
        ident_result.is_ok(),
    ...
    let executer_result = orch.rat().run_executer("BTC", 65_000.0).await;
    assert!(
