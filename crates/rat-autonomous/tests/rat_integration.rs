...
use rat_core::{Config, DisciplineRules, MemoryStore, OhlcvBar, TradeDirection};
...
fn simulate_outcome_recording(state: &SharedState, symbol: &str, pnl: f64, was_win: bool) {
    ...
    let votes = vec![rat_core::SkillVote {
        skill_name: "SentimentAnalyzer".to_string(),
        direction: rat_core::SkillDirection::Bullish,
        weight: 0.30,
        confidence: 0.75,
        score: 0.70,
    }];
    ...
    for v in &votes {
        let sp = rat_autonomous::episode_store::SkillPerformanceRow {
            id: 0,
    ...
    let closed = rat_autonomous::episode_store::ClosedEpisode {
        id: episode_id.clone(),
