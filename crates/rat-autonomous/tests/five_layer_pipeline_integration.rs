...
//
// Run: cargo test -p rat-autonomous --test five_layer_pipeline_integration

use chrono::Utc;
use std::fs;
use rat_autonomous::types::OpenPosition;
use rat_autonomous::{AutonomousOrchestrator, SharedState};
...
async fn seed_aggregated_signal(state: &SharedState, conviction: f64) {
    ...
    use rat_core::skill_aggregator::AggregatedSignal;
...
async fn test_high_priority_overrides_medium() {
    ...
    *orch.state.market_data.market_regime.write().await =
    ...
    let gate = rat_autonomous::hard_rules_gate::HardRulesGate::new(orch.state.clone());
    let gate_result = gate.evaluate("ETH").await;

    assert!(!gate_result.passed, "Gate should block");
    assert_eq!(
        gate_result.highest_failed_priority,
        Some(rat_autonomous::types::RulePriority::High),
...
async fn test_identifier_feeds_debate_context() {
    ...
    *orch.state.market_data.market_regime.write().await =
    ...
    let ident_result = orch.rat().run_identifier("SOL", 180.0, 1).await;
    assert!(
        ident_result.is_ok(),
...
async fn test_medium_only_block_stops_pipeline() {
    ...
    *orch.state.market_data.market_regime.write().await =
    ...
    assert_eq!(
        gate_result.highest_failed_priority,
        Some(rat_autonomous::types::RulePriority::Medium),
