...
impl AutonomousOrchestrator {
    /// Convenience accessor for the Rat agent hierarchy.
    /// Panics if init_rat() has not been called after construction.
    pub fn rat(&self) -> &Rat {
        self.rat
            .as_ref()
            .expect("Rat not initialized — call init_rat() after construction")
    }

    /// Initialize the Rat agent hierarchy (must be called once after construction).
    /// Uses Arc::clone() for zero-copy sharing — no agent state is duplicated.
    pub fn init_rat(&mut self) {
        self.rat = Some(Rat::from_orchestrator(self));
    }
    ...
    pub fn new(state: SharedState) -> Self {
        Self {
            ...
            state,
            rat: None, // set to Some(Rat::from_orchestrator(self)) after construction
        }
    }
