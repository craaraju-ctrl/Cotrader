...
use rat_core::{DisciplineRules, OhlcvBar};
...
pub fn calculate_risk_reward(
    ...
    stop: f64,
    target: f64,
    direction: rat_core::TradeDirection,
) -> f64 {
    ...
    match direction {
        rat_core::TradeDirection::Long if target > entry => reward / risk,
        rat_core::TradeDirection::Short if target < entry => reward / risk,
        _ => 0.0,
    }
}
...
pub fn compute_autonomous_levels(
    _symbol: &str,
    current_price: f64,
    pivots: &rat_core::PivotLevels,
    _patterns: &[rat_core::CandlestickPattern], // patterns come from MI state; kept for future extension
    ...
    forced_direction: Option<rat_core::TradeDirection>,
) -> (f64, f64, f64, f64) {
    // entry, sl, tp, rr
    let mut direction = forced_direction.unwrap_or(rat_core::TradeDirection::Long);
    ...
    if let Some(agg) = aggregated_signal {
        if let Some(forced) = forced_direction {
            ...
            if (forced == rat_core::TradeDirection::Long && agg.is_bullish(None))
                || (forced == rat_core::TradeDirection::Short && agg.is_bearish(None))
            {
    ...
    let entry = if direction == rat_core::TradeDirection::Long {
    ...
    let sl = if direction == rat_core::TradeDirection::Long {
    ...
    let tp = if direction == rat_core::TradeDirection::Long {
    ...
    let final_tp = if rr < min_rr {
        if direction == rat_core::TradeDirection::Long {
            entry + risk * min_rr
        } else {
            entry - risk * min_rr
        }
