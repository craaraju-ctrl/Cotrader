...
pub struct DebateContext {
    ...
    pub aggregated_signal: Option<rat_core::AggregatedSignal>,
...
impl DebateLayer {
    ...
    async fn assemble_context(
        ...
        let (vector_memory_matches, skill_votes_bullish, skill_votes_bearish, skill_votes_neutral) = {
            ...
            for v in votes.iter() {
                match v.direction {
                    rat_core::agent::SkillDirection::Bullish => bull += 1,
                    rat_core::agent::SkillDirection::Bearish => bear += 1,
                    _ => neut += 1,
                }
            }
    ...
    async fn build_signal_from_verdict(
        ...
        let pivots = rat_core::calculate_pivot_points(
            ctx.price * 1.01,
            ctx.price * 0.99,
            ctx.price * 0.998,
            rules.pivot_method,
        ...
        if !levels_valid {
            stop_loss = if direction == rat_core::TradeDirection::Long {
                entry - min_stop_dist
            } else {
                entry + min_stop_dist
            };
            take_profit = if direction == rat_core::TradeDirection::Long {
