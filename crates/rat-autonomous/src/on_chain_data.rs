...
use std::error::Error;
use rat_core::{skills::AgentSkill, AgentInput, AgentOutput};
...
impl OnChainData {
    ...
    fn calculate_volume_signal(&self, bars: &[rat_core::OhlcvBar]) -> f64 {
        if bars.len() < 20 {
            return 0.5;
        }
    ...
    fn calculate_price_signal(&self, bars: &[rat_core::OhlcvBar]) -> f64 {
        if bars.len() < 10 {
            return 0.5;
        }
    ...
    fn calculate_volatility_signal(&self, bars: &[rat_core::OhlcvBar]) -> f64 {
        if bars.len() < 20 {
            return 0.5;
        }
        ...
        if let AgentInput::ConfluenceRequest { context } = input {
            ...
            Ok(AgentOutput::SkillResult {
                ...
                direction: if score > 0.55 {
                    rat_core::agent::SkillDirection::Bullish
                } else if score < 0.45 {
                    rat_core::agent::SkillDirection::Bearish
                } else {
                    rat_core::agent::SkillDirection::Neutral
                },
                weight: 0.15,
            })
        } else {
