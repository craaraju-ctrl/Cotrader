...
use rat_core::paper_engine::{OrderRequest, OrderStatus, OrderType, TradingMode};
use rat_core::{Agent, AgentInput, AgentOutput, AgentTier, TradeOutcome, TradingEpisode};
...
impl ExecutionCoordinatorAgent {
    ...
    async fn compliance_check(
        ...
        let proposal = ComplianceProposal {
            ...
            direction: if signal.direction == rat_core::TradeDirection::Long {
                "BUY"
            } else {
                "SELL"
            }
    ...
    pub async fn execute_paper_trade_with_chain(
        ...
        let direction_str = if signal.direction == rat_core::TradeDirection::Long {
            "BUY"
        } else {
            "SELL"
        };
        ...
        let effective_sl = match induced_regret_sl_pct() {
            Some(pct) => {
                let tight = match signal.direction {
                    rat_core::TradeDirection::Long => effective_entry * (1.0 - pct / 100.0),
                    rat_core::TradeDirection::Short => effective_entry * (1.0 + pct / 100.0),
                };
                println!(
        ...
        println!(
            ...
            signal.symbol,
            if signal.direction == rat_core::TradeDirection::Long {
                "BUY"
            } else {
                "SELL"
            },
    ...
    pub async fn refresh_position_prices(&self) {
        ...
        let open_value: f64 = portfolio
            ...
            .map(|p| match p.direction {
                rat_core::TradeDirection::Long => p.quantity * p.current_price,
                rat_core::TradeDirection::Short => {
    ...
    async fn check_and_exit_positions(&self) -> Result<Vec<String>, Box<dyn Error + Send + Sync>> {
                        ...
                        Ok(pnl) => {
                            ...
                            let closed = rat_core::ClosedTrade {
                                ...
                                close_reason: match exit_reason {
                                    "stop_loss" => rat_core::CloseReason::StopLoss,
                                    "take_profit" => rat_core::CloseReason::TakeProfit,
                                    _ => rat_core::CloseReason::Manual,
                                },
        ...
        for pos in &positions_snapshot {
            ...
            let stop_hit = match pos.direction {
                rat_core::TradeDirection::Long => current_price <= pos.stop_loss,
                rat_core::TradeDirection::Short => current_price >= pos.stop_loss,
            };

            let tp_hit = match pos.direction {
                rat_core::TradeDirection::Long => current_price >= pos.take_profit,
                rat_core::TradeDirection::Short => current_price <= pos.take_profit,
            };
                ...
                match pm.close_position(&pos.symbol, pos.stop_loss).await {
                    Ok(pnl) => {
                        ...
                        let closed = rat_core::ClosedTrade {
                            ...
                            realized_pnl: pnl,
                            realized_pnl_pct: (pnl / (pos.entry_price * pos.quantity)).abs(),
                            close_reason: rat_core::CloseReason::StopLoss,
                            opened_at: pos.entry_time,
                            closed_at: Utc::now(),
                            duration_secs: (Utc::now() - pos.entry_time).num_seconds(),
                ...
                match pm.close_position(&pos.symbol, pos.take_profit).await {
                    Ok(pnl) => {
                        ...
                        let closed = rat_core::ClosedTrade {
                            ...
                            realized_pnl: pnl,
                            realized_pnl_pct: (pnl / (pos.entry_price * pos.quantity)).abs(),
                            close_reason: rat_core::CloseReason::TakeProfit,
                            opened_at: pos.entry_time,
                            closed_at: Utc::now(),
                            duration_secs: (Utc::now() - pos.entry_time).num_seconds(),
                            strategy: Some("debate_driven".to_string()),
                ...
                match pm.close_position(&pos.symbol, current_price).await {
                    Ok(pnl) => {
                        ...
                        let closed = rat_core::ClosedTrade {
                            ...
                            realized_pnl: pnl,
                            realized_pnl_pct: (pnl / (pos.entry_price * pos.quantity)).abs(),
                            close_reason: rat_core::CloseReason::Manual,
                            opened_at: pos.entry_time,
                            closed_at: Utc::now(),
                            duration_secs: (Utc::now() - pos.entry_time).num_seconds(),
                            strategy: Some("debate_driven".to_string()),
        ...
        match input {
            Some(AgentInput::ConfluenceRequest { context }) => {
                let signal = TradeSignal {
                    ...
                    direction: rat_core::TradeDirection::Long,
...
async fn build_pre_trade_snapshot(
    ...
    let direction_str = match signal.direction {
        rat_core::TradeDirection::Long => "BUY",
        rat_core::TradeDirection::Short => "SELL",
    };
