...
impl MarketIntelligenceAgent {
    ...
    pub async fn analyze_market(
        ...
        symbol: &str,
        price: f64,
    ) -> Result<(f64, rat_core::PivotLevels), Box<dyn Error + Send + Sync>> {
        ...
        let skills: Vec<Box<dyn rat_core::skills::AgentSkill>> = vec![
        ...
        for skill in &skills {
            if skill.is_available() {
                if let Ok(rat_core::AgentOutput::SkillResult {
                    name,
                    score,
                    note,
                    confidence,
                    ...
                    .await
                {
                    ...
                    skill_outputs.push(rat_core::AgentOutput::SkillResult {
                        name: name.clone(),
                        score,
                        note: note.clone(),
                        confidence,
        ...
        for output in &skill_outputs {
            if let rat_core::AgentOutput::SkillResult { name, score, .. } = output {
