...
impl EvolvedMetaControl {
    ...
    pub fn evaluate_and_adapt(
        ...
        let recent_closed = self.db.load_recent_closed_trades(15, None).unwrap_or_default();
        if recent_closed.len() < 15 {
            return None;
        }

        let average_regret: f64 = recent_closed.iter().map(|t| t.regret_score).sum::<f64>() / recent_closed.len() as f64;
        if average_regret > 0.70 {
            ...
            // Compute past baselines using actual P&L (not regret score)
            let wins = recent_closed.iter().filter(|t| t.pnl > 0.0).count();
            let baseline_win_rate = wins as f64 / recent_closed.len() as f64;
