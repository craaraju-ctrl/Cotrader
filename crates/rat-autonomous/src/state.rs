...
use rat_core::{CandlestickPattern, MultiTfPatternConfirmation};
...
impl SharedState {
    ...
    fn graph_recall_from_context(
        ...
        kg: &KnowledgeGraph,
        query_context: &str,
    ) -> rat_core::graph_rag::GraphRecallResult {
        ...
        match (found_symbol, found_regime) {
            ...
            (Some(sym), None) => {
                ...
                let start = rat_core::graph_rag::GraphNode::Symbol(sym.to_string());
                kg.query_relationship(&start, None, 2)
            }
            (None, Some(reg)) => {
                // Regime only — 2-hop traversal from regime node
                let start = rat_core::graph_rag::GraphNode::Regime(reg.to_string());
                kg.query_relationship(&start, None, 2)
            }
            (None, None) => {
