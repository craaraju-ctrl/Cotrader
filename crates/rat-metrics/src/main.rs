// ═══════════════════════════════════════════════════════════════════════════════
// rat-metrics — Multi-Layer Monitoring & Alerting
//
...
const DB_PATH: &str = "~/.rat/metrics.db";
const CONFIG_PATH: &str = "~/.rat/metrics.toml";
const PROMETHEUS_NAMESPACE: &str = "rat";
...
impl MetricsState {
    ...
    fn send_alert(&self, title: &str, message: &str) {
        let full_msg = format!("[rat-metrics] {} — {}", title, message);
            ...
            (&self.alert_config.smtp_host, &self.alert_config.smtp_to)
        {
            ...
            let _from = self
                ...
                .smtp_from
                .clone()
                .unwrap_or_else(|| "rat@metrics.local".to_string());
            let body_text = format!("Subject: [rat] {}\n\n{}", title, full_msg);
    ...
    fn format_report_text(&self, report: &PerformanceReport) -> String {
        format!(
            r#"📊 rat Weekly Performance Report
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Period: {} — {}
...
fn write_default_config(path: &str) {
    ...
    let content = format!(
        "# rat-metrics alert configuration\n\
...
async fn main() {
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "rat_metrics=info".into()),
        )
        .init();
    ...
    let app = Router::new()
        ...
        .route(
            "/health",
            get(|| async { Json(serde_json::json!({"status": "ok", "service": "rat-metrics"})) }),
        )
...
mod tests {
    ...
    fn test_prometheus_format() {
        ...
        assert!(output.contains("rat_uptime_seconds"));
        assert!(output.contains("rat_win_rate"));
        assert!(output.contains("rat_sharpe_ratio"));
    }
}
