...
pub fn detect_patterns(bars: &[OhlcvBar]) -> Vec<CandlestickPattern> {
    ...
    // New: Inside Bar (consolidation signal)
    if let Some(pattern) = detect_inside_bar(bars) {
        patterns.push(pattern);
    }
    // New: Pin Bar (long wick rejection)
    if let Some(pattern) = detect_pin_bar(bars) {
        patterns.push(pattern);
    }
    // New: Fakey pattern (inside bar false breakout)
    if let Some(pattern) = detect_fakey(bars) {
        patterns.push(pattern);
    }
    // New: Three Inside Up/Down (3-bar reversal)
    if let Some(pattern) = detect_three_inside_up(bars) {
        patterns.push(pattern);
    }
    if let Some(pattern) = detect_three_inside_down(bars) {
        patterns.push(pattern);
    }
    // New: Separating Lines (continuation)
    if let Some(pattern) = detect_separating_lines(bars) {
        patterns.push(pattern);
    }
...
fn detect_inside_bar(bars: &[OhlcvBar]) -> Option<CandlestickPattern> {
    if bars.len() < 2 {
        return None;
    }
    let prev = &bars[bars.len() - 2];
    ...
    if curr.high < prev.high && curr.low > prev.low {
        let strength = 0.5 + (body_size(curr) / body_size(prev).max(0.0001)) * 0.3;
        Some(CandlestickPattern {
            name: "Inside Bar".to_string(),
            direction: "neutral".to_string(),
            strength: strength.clamp(0.0, 0.85),
            bar_index: bars.len() - 1,
        })
    } else {
        None
...
/// Bullish pin bar: long lower wick, closes near high.
/// Bearish pin bar: long upper wick, closes near low.
fn detect_pin_bar(bars: &[OhlcvBar]) -> Option<CandlestickPattern> {
    if bars.is_empty() {
        return None;
    }
    let b = bars.last()?;
    let body = body_size(b);
    let full_range = (b.high - b.low).max(0.0001);
    let upper_wick = b.high - b.open.max(b.close);
    let lower_wick = b.open.min(b.close) - b.low;

    // Body must be small (< 30% of range)
    if body / full_range > 0.3 {
        return None;
    }

    // Bullish pin bar: long lower wick (> 60% of range), closes near high
    if lower_wick / full_range > 0.6 && (b.close - b.low) / full_range > 0.7 {
        let strength = 0.6 + (lower_wick / full_range - 0.6) * 0.5;
    ...
    // Bearish pin bar: long upper wick (> 60% of range), closes near low
    if upper_wick / full_range > 0.6 && (b.high - b.close) / full_range > 0.7 {
        let strength = 0.6 + (upper_wick / full_range - 0.6) * 0.5;
        return Some(CandlestickPattern {
...
/// Fakey Pattern — inside bar followed by a false breakout that reverses.
fn detect_fakey(bars: &[OhlcvBar]) -> Option<CandlestickPattern> {
    if bars.len() < 3 {
        return None;
    }
    let b1 = &bars[bars.len() - 3];
    let b2 = &bars[bars.len() - 2]; // inside bar
    let b3 = bars.last()?;
    ...
    // Bullish fakey: b3 breaks below b2 low then closes above b2 close
    if b3.low < b2.low && b3.close > b2.close && is_bullish(b3) {
        let strength = 0.65 + (body_size(b3) / (b1.high - b1.low).max(0.0001)) * 0.2;
        return Some(CandlestickPattern {
    ...
    // Bearish fakey: b3 breaks above b2 high then closes below b2 close
    if b3.high > b2.high && b3.close < b2.close && is_bearish(b3) {
        let strength = 0.65 + (body_size(b3) / (b1.high - b1.low).max(0.0001)) * 0.2;
        return Some(CandlestickPattern {
...
/// Three Inside Up — bearish candle, harami, then bullish confirmation.
fn detect_three_inside_up(bars: &[OhlcvBar]) -> Option<CandlestickPattern> {
    if bars.len() < 3 {
        return None;
    }
    let b1 = &bars[bars.len() - 3];
    ...
    if is_bearish(b1) && is_bullish(b3) && b3.close > b1.open {
        // b2 should be inside b1 (harami)
        if b2.high < b1.high && b2.low > b1.low {
            return Some(CandlestickPattern {
                name: "Three Inside Up".to_string(),
                direction: "bullish".to_string(),
                strength: 0.75,
                bar_index: bars.len() - 1,
            });
...
/// Three Inside Down — bullish candle, harami, then bearish confirmation.
fn detect_three_inside_down(bars: &[OhlcvBar]) -> Option<CandlestickPattern> {
    if bars.len() < 3 {
        return None;
    }
    let b1 = &bars[bars.len() - 3];
    ...
    if is_bullish(b1) && is_bearish(b3) && b3.close < b1.open {
        if b2.high < b1.high && b2.low > b1.low {
            return Some(CandlestickPattern {
                name: "Three Inside Down".to_string(),
                direction: "bearish".to_string(),
                strength: 0.75,
                bar_index: bars.len() - 1,
            });
...
/// Separating Lines — two candles with same open, opposite directions.
fn detect_separating_lines(bars: &[OhlcvBar]) -> Option<CandlestickPattern> {
    if bars.len() < 2 {
        return None;
    }
    let b1 = &bars[bars.len() - 2];
    let b2 = bars.last()?;

    let open_diff = (b1.open - b2.open).abs() / b1.open.max(0.0001);
    if open_diff > 0.001 {
        return None; // Opens must be nearly identical
    }

    // Bullish separating lines: bearish then bullish (continuation in uptrend)
    if is_bearish(b1) && is_bullish(b2) {
        return Some(CandlestickPattern {
            name: "Bullish Separating Lines".to_string(),
            direction: "bullish".to_string(),
            strength: 0.65,
            bar_index: bars.len() - 1,
        });
    ...
    // Bearish separating lines: bullish then bearish (continuation in downtrend)
    if is_bullish(b1) && is_bearish(b2) {
        return Some(CandlestickPattern {
            name: "Bearish Separating Lines".to_string(),
            direction: "bearish".to_string(),
            strength: 0.65,
            bar_index: bars.len() - 1,
        });
