...
impl std::fmt::Display for AdvancedPatternType {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            AdvancedPatternType::AscendingTriangle => write!(f, "Ascending Triangle"),
            AdvancedPatternType::DescendingTriangle => write!(f, "Descending Triangle"),
            AdvancedPatternType::SymmetricTriangle => write!(f, "Symmetric Triangle"),
            AdvancedPatternType::CupAndHandle => write!(f, "Cup and Handle"),
        }
    }
}
...
pub fn detect_advanced_patterns(bars: &[OhlcvBar]) -> Vec<AdvancedPattern> {
    ...
    if let Some(t) = detect_triangle(bars) {
        patterns.push(AdvancedPattern {
            pattern_type: match t.triangle_type.as_str() {
                "ascending" => AdvancedPatternType::AscendingTriangle,
                "descending" => AdvancedPatternType::DescendingTriangle,
                _ => AdvancedPatternType::SymmetricTriangle,
            },
            direction: t.breakout_direction.clone(),
            strength: t.strength,
            target_price: Some(t.target),
            invalidation: Some(t.invalidation),
            description: format!(
                "{} Triangle: upper={:.2}, lower={:.2}, target={:.2}",
                t.triangle_type, t.upper_line, t.lower_line, t.target
            ),
            bar_range: (0, bars.len() - 1),
        });
    }
    ...
    if let Some(ch) = detect_cup_and_handle(bars) {
        patterns.push(AdvancedPattern {
            pattern_type: AdvancedPatternType::CupAndHandle,
            direction: "bullish".into(),
            strength: ch.strength,
            target_price: Some(ch.target),
            invalidation: Some(ch.invalidation),
            description: format!(
                "Cup and Handle: cup_depth={:.1}%, handle={:.1}%, target={:.2}",
                ch.cup_depth_pct * 100.0,
                ch.handle_depth_pct * 100.0,
                ch.target
            ),
            bar_range: (0, bars.len() - 1),
        });
...
/// Triangle pattern — converging trendlines (ascending, descending, symmetric)
#[derive(Debug, Clone)]
pub struct TrianglePattern {
    pub triangle_type: String,
    pub upper_line: f64,
    pub lower_line: f64,
    pub target: f64,
    pub invalidation: f64,
...
/// Detect triangle pattern using linear regression on highs and lows.
fn detect_triangle(bars: &[OhlcvBar]) -> Option<TrianglePattern> {
    if bars.len() < 20 {
        return None;
    }
    ...
    // Fit trendlines
    let upper_slope = (highs.last()?.1 - highs.first()?.1) / (highs.last()?.0 - highs.first()?.0) as f64;
    let lower_slope = (lows.last()?.1 - lows.first()?.1) / (lows.last()?.0 - lows.first()?.0) as f64;

    let upper_line = highs.last()?.1;
    let lower_line = lows.last()?.1;
    let range = upper_line - lower_line;
    ...
    let triangle_type = if upper_slope.abs() < 0.001 && lower_slope > 0.001 {
        "ascending".to_string()
    } else if upper_slope < -0.001 && lower_slope.abs() < 0.001 {
        "descending".to_string()
    } else if upper_slope < -0.001 && lower_slope > 0.001 {
        "symmetric".to_string()
    } else {
        return None;
    };

    // Breakout direction based on triangle type
    let breakout_direction = match triangle_type.as_str() {
        "ascending" => "bullish".to_string(),
        "descending" => "bearish".to_string(),
        _ => "neutral".to_string(),
    };

    let target = if breakout_direction == "bullish" {
        upper_line + range
    } else if breakout_direction == "bearish" {
        lower_line - range
    } else {
        (upper_line + lower_line) / 2.0
    };

    let strength = 0.7 + (1.0 - range / upper_line) * 0.2;
    ...
    Some(TrianglePattern {
        ...
        lower_line,
        target,
        invalidation: if breakout_direction == "bullish" { lower_line } else { upper_line },
        breakout_direction,
...
#[derive(Debug, Clone)]
pub struct CupAndHandlePattern {
...
fn detect_cup_and_handle(bars: &[OhlcvBar]) -> Option<CupAndHandlePattern> {
    if bars.len() < 30 {
        return None;
    }
    ...
    // Find the highest high (rim) and lowest low (cup bottom)
    let rim = recent.iter().map(|b| b.high).fold(f64::MIN, f64::max);
    let cup_bottom = recent.iter().map(|b| b.low).fold(f64::MAX, f64::min);
    let cup_depth = rim - cup_bottom;

    if cup_depth <= 0.0 || cup_depth / rim < 0.05 {
        return None; // Too shallow
    }

    // Check if recent bars form a handle (small pullback from rim)
    let last_5 = &recent[recent.len() - 5..];
    let handle_high = last_5.iter().map(|b| b.high).fold(f64::MIN, f64::max);
    let handle_low = last_5.iter().map(|b| b.low).fold(f64::MAX, f64::min);
    let handle_depth = handle_high - handle_low;
    ...
    let cup_depth_pct = cup_depth / rim;
    let handle_depth_pct = handle_depth / rim;
    let target = rim + cup_depth; // Measured move
    let strength = 0.7 + (1.0 - handle_depth_pct / cup_depth_pct) * 0.2;

    Some(CupAndHandlePattern {
        cup_depth_pct,
        handle_depth_pct,
        target,
