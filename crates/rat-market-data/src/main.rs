// ═══════════════════════════════════════════════════════════════════════════════
// rat-market-data — Standalone Market Data Service
//
...
//
// Architecture:
//   rat-market-data ──(NATS publish)──→ Other services
//       ↑                            ↑
//   Binance API                  Yahoo Finance
//
// Subjects published:
//   rat.market.price.{symbol}   — Latest price tick
//   rat.market.ohlcv.{symbol}   — OHLCV bar data
//   rat.health.market-data      — Health check status
//
// Listens on:
...
use rat_eventbus::{subjects, EventBus, RatEvent};
use rat_eventbus::{MarketOhlcvBar, MarketPriceEvent};
        ...
        loop {
                            ...
                            if !ohlcv_bars.is_empty() {
                                let _ = bus
                                    .publish(
                                        ...
                                        &RatEvent::MarketOhlcv(
                                            rat_eventbus::MarketOhlcvEvent {
                                                symbol: symbol.clone(),
            ...
            let _ = self
                ...
                .publish(
                    ...
                    &RatEvent::Health(rat_eventbus::HealthEvent {
...
async fn fetch_klines(
    ...
    let klines = rat_core::fetch_klines(client, symbol, interval, limit).await?;
    Ok(klines
        .into_iter()
        .map(|b| OhlcvBar {
...
async fn main() {
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "rat_market_data=info".into()),
        )
        .init();
    ...
    tokio::spawn(async move {
        while let Some((_subject, event)) = control_sub.recv().await {
            if let RatEvent::SystemControl(ctl) = &event {
                info!(
                    command = %ctl.command,
                    reason = %ctl.reason,
