// ═══════════════════════════════════════════════════════════════════════════════
// rat-eventbus — Microservices Event Bus
//
...
//   rat.<domain>.<action>[.<detail>]
//
// Core subjects:
//   rat.market.price.{symbol}      — Price updates (published by market-data)
//   rat.market.ohlcv.{symbol}      — OHLCV bar data
//   rat.signal.{symbol}            — Trade signals from pipeline
//   rat.execution.order             — Order execution requests
//   rat.execution.filled            — Order fill confirmations
//   rat.portfolio.snapshot          — Portfolio state snapshots
//   rat.health.{service}           — Health check events
//   rat.system.control             — Start/stop/shutdown commands
//   rat.cot.{symbol}               — Chain-of-thought entries
// ═══════════════════════════════════════════════════════════════════════════════
...
pub trait EventBus: std::fmt::Debug + Send + Sync {
    ...
    async fn request(
        ...
        event: &RatEvent,
        _timeout_secs: u64,
    ) -> Result<RatEvent, Box<dyn Error + Send + Sync>>;
...
pub trait EventStream: Send + Sync {
    async fn recv(&mut self) -> Option<(String, RatEvent)>;
}
...
impl NatsEventBus {
    ...
    fn serialize(event: &RatEvent) -> Result<bytes::Bytes, Box<dyn Error + Send + Sync>> {
        Ok(serde_json::to_vec(event)?.into())
    }

    /// Helper: deserialize event from bytes.
    fn deserialize(data: &[u8]) -> Result<RatEvent, Box<dyn Error + Send + Sync>> {
...
impl EventBus for NatsEventBus {
    ...
    async fn respond(
        ...
        let subscriber = self
            .client
            .queue_subscribe(subject.to_string(), "rat-workers".to_string())
            .await?;
        Ok(Box::new(NatsStream { inner: subscriber }))
    }
...
impl EventBus for InMemoryEventBus {
    ...
    async fn subscribe(
        ...
        let (tx, rx) = mpsc::channel::<(String, RatEvent)>(256);
...
pub mod subjects {
    pub fn market_price(symbol: &str) -> String {
        format!("rat.market.price.{}", symbol.to_lowercase())
    }
    pub fn market_ohlcv(symbol: &str) -> String {
        format!("rat.market.ohlcv.{}", symbol.to_lowercase())
    }
    pub fn signal(symbol: &str) -> String {
        format!("rat.signal.{}", symbol.to_lowercase())
    }
    pub fn execution_order() -> String {
        "rat.execution.order".to_string()
    }
    pub fn execution_filled() -> String {
        "rat.execution.filled".to_string()
    }
    pub fn portfolio_snapshot() -> String {
        "rat.portfolio.snapshot".to_string()
    }
    pub fn health(service: &str) -> String {
        format!("rat.health.{}", service.to_lowercase())
    }
    pub fn system_control() -> String {
        "rat.system.control".to_string()
    }
    pub fn cot(symbol: &str) -> String {
        format!("rat.cot.{}", symbol.to_lowercase())
    }

    // Wildcard patterns for subscriptions
    pub fn all_market_events() -> String {
        "rat.market.*.*".to_string()
    }
    pub fn all_signal_events() -> String {
        "rat.signal.*".to_string()
    }
...
mod tests {
    ...
    async fn test_in_memory_publish_subscribe() {
        ...
        let mut sub = bus.subscribe("rat.market.price.BTC").await.unwrap();

        let event = RatEvent::MarketPrice(MarketPriceEvent {
        ...
        bus.publish("rat.market.price.BTC", &event).await.unwrap();
        ...
        assert_eq!(received.0, "rat.market.price.BTC");
        match received.1 {
            RatEvent::MarketPrice(price) => {
    ...
    async fn test_in_memory_wildcard_subscription() {
        let bus = InMemoryEventBus::new();
        let mut sub = bus.subscribe("rat.market.price.*").await.unwrap();

        bus.publish(
            "rat.market.price.BTC",
    ...
    async fn test_signal_event_roundtrip() {
        ...
        bus.publish("rat.signal.SOL", &event).await.unwrap();
    ...
    async fn test_system_control_event() {
        let bus = InMemoryEventBus::new();
        let mut sub = bus.subscribe("rat.system.control").await.unwrap();

        let event = RatEvent::SystemControl(SystemControlEvent {
        ...
        bus.publish("rat.system.control", &event).await.unwrap();
