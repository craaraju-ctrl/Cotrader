...
/// Toast notification (auto-expiring)
#[derive(Debug, Clone)]
pub struct Toast {
    pub message: String,
    pub level: AlertLevel,
...
pub struct App {
    ...
    /// Rendered in the footer so a dead endpoint is VISIBLE, not silent
    /// (the split-brain bug survived because nothing showed it).
    pub source_status: std::collections::HashMap<String, (bool, std::time::Instant)>,
    ...
    /// Toast notifications (auto-expiring)
    pub toasts: VecDeque<Toast>,
    /// Index of last pipeline event shown as toast (prevents spam)
    pub last_shown_event_idx: usize,
}
...
impl App {
    ...
    /// Clamp scroll_offset to prevent unbounded scrolling
    ...
    /// Show a toast notification (auto-expires after 3 seconds)
    pub fn show_toast(&mut self, message: &str, level: AlertLevel) {
    ...
    pub fn clean_toasts(&mut self) {
        let now = std::time::Instant::now();
        self.toasts.retain(|t| now.duration_since(t.created_at).as_secs() < 3);
    }
