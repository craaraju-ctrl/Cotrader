...
impl Component for HealthComponent {
    fn render(&self, frame: &mut Frame, area: Rect, app: &App) {
        let has_service_status = app.service_status.is_some();
        let chunks = Layout::default()
            .direction(Direction::Vertical)
            .constraints(if has_service_status {
                vec![
                    Constraint::Length(6),
                    Constraint::Min(6),
                    Constraint::Length(6),
        ...
        if has_service_status {
            self.render_service_status_json(frame, chunks[2], app);
        }
    }
}
...
impl HealthComponent {
    ...
    fn render_service_status_json(&self, frame: &mut Frame, area: Rect, app: &App) {
        let json_str = app.service_status.as_ref()
            .map(|v| serde_json::to_string_pretty(v).unwrap_or_else(|_| "Invalid JSON".to_string()))
            .unwrap_or_default();

        // Truncate long output to fit the area
        let max_lines = area.height.saturating_sub(2) as usize;
        let lines: Vec<Line> = json_str.lines()
            .take(max_lines)
