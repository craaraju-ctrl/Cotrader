...
impl AgentsComponent {
    fn render_agent_tree(&self, frame: &mut Frame, area: Rect, app: &App) {
        // Filter agents by search query if active
        let filtered: Vec<&crate::app::AgentStatus> = if app.show_search && !app.search_query.is_empty() {
            let q = app.search_query.to_uppercase();
            app.agents.iter().filter(|a| a.name.to_uppercase().contains(&q)).collect()
        } else {
            app.agents.iter().collect()
        };

        let items: Vec<ListItem> = filtered.iter().enumerate().map(|(i, agent)| {
