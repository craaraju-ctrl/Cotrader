...
impl Default for PaperEngineConfig {
    fn default() -> Self {
        ...
        // Convert default/specified USD balance to INR if base currency is INR
        let initial_balance = if base_currency == "INR" && initial_balance <= 100_000.0 {
            initial_balance * 83.0 // convert $100k USD to 8,300,000 INR
        } else {
            initial_balance
        };
        ...
        Self {
            base_currency,
            initial_balance,
            // PaperEngine only ever runs in paper mode, so these are
            // deliberately relaxed (95%, not the live 4%/1-25 cap) to give
            // paper runs enough trade frequency to validate strategy logic —
            // matching the paper-mode convention used in strategy_decision.rs
            // and portfolio_manager.rs. Kept at 95% rather than a flat 100%
            // so there's always a small buffer against rounding/slippage.
            max_position_size_pct: 95.0,
            max_daily_loss_pct: 3.0,
            max_drawdown_pct: 10.0,
            max_concentration_pct: 95.0,
            max_portfolio_heat_pct: 95.0,
            max_leverage: 1.0,
            slippage_model: SlippageModel::Fixed(0.01), // 1 paisa / cent slippage
            commission_pct: 0.03,
...
impl PaperEngine {
    ...
    /// Normalize a price from native asset currency to the portfolio base currency.
    pub fn normalize_price(&self, symbol: &str, price: f64) -> f64 {
        let is_crypto = crate::is_crypto_symbol(symbol);
        if self.config.base_currency == "INR" {
            if is_crypto {
                price * 83.0
            } else {
                price
    ...
    pub async fn place_order(
        ...
        let normalized_fill_price = self.normalize_price(&request.symbol, fill_price);
        let normalized_stop_loss = request
            .stop_loss
            .map(|val| self.normalize_price(&request.symbol, val))
            .unwrap_or(0.0);
        let normalized_take_profit = request
            .take_profit
            .map(|val| self.normalize_price(&request.symbol, val))
            .unwrap_or(0.0);
        ...
        let effective_cost = normalized_fill_price * effective_qty as f64;
        ...
        let position = Position {
            ...
            current_price: normalized_fill_price,
    ...
    pub async fn update_price(&self, symbol: &str, market_price: f64) -> Vec<ClosedTrade> {
        ...
        let normalized_price = self.normalize_price(symbol, market_price);
        ...
        while i < portfolio.positions.len() {
            ...
            portfolio.positions[i].current_price = normalized_price;
            ...
            if pos_clone.stop_loss > 0.0 {
                let stop_hit = match pos_clone.direction {
                    TradeDirection::Long => normalized_price <= pos_clone.stop_loss,
                    TradeDirection::Short => normalized_price >= pos_clone.stop_loss,
                };
                if stop_hit {
            ...
            if pos_clone.take_profit > 0.0 {
                let tp_hit = match pos_clone.direction {
                    TradeDirection::Long => normalized_price >= pos_clone.take_profit,
                    TradeDirection::Short => normalized_price <= pos_clone.take_profit,
                };
                if tp_hit {
                    let closed_trade = self.close_position_internal(
                        &mut portfolio,
    ...
    pub async fn close_position(
        ...
        let symbol = portfolio.positions[idx].symbol.clone();
        let normalized_exit_price = self.normalize_price(&symbol, exit_price);
        Ok(self.close_position_internal(
            &mut portfolio,
            idx,
            CloseReason::Manual,
    ...
    pub async fn check_risk(&self, symbol: &str, estimated_cost: f64) -> RiskCheckResult {
        ...
        // Enforce trade value limits check:
        // Crypto (USD based) must not be worth more than 1000.0 USD.
        // Stocks (INR based) must not be worth more than 100000.0 INR.
        let is_crypto = crate::is_crypto_symbol(symbol);
        let max_trade_limit = if is_crypto { 1000.0 } else { 100000.0 };
        let currency_label = if is_crypto { "USD" } else { "INR" };
        if estimated_cost > max_trade_limit {
            result.max_position_size_ok = false;
            result.passed = false;
            result.warnings.push(format!(
                "Trade size of {:.2} {} exceeds native limit of {:.2} {}",
                estimated_cost, currency_label, max_trade_limit, currency_label
            ));
        }

        // Normalize estimated_cost to base currency for portfolio equity level checks
        let estimated_cost_normalized = self.normalize_price(symbol, estimated_cost);

        // Max position size
        let max_pos_value = portfolio.equity * (self.config.max_position_size_pct / 100.0);
        if estimated_cost_normalized > max_pos_value {
            result.max_position_size_ok = false;
            result.passed = false;
            result.warnings.push(format!(
                "Position size of {:.2} exceeds max {:.2} ({:.1}% of equity)",
                estimated_cost_normalized, max_pos_value, self.config.max_position_size_pct
            ));
        }
        ...
        let existing_exposure: f64 = portfolio
            ...
            .sum();
        let total_exposure = existing_exposure + estimated_cost_normalized;
