...
use rat_autonomous::orchestrator_struct::AutonomousOrchestrator;
use rat_core::paper_engine::{BrokerRegistry, OrderRequest, OrderType};
...
impl RuntimeEngine {
    ...
    /// Update or create 1m OHLCV bar using rat_core::OhlcvBar (which uses String timestamps).
    ...
    fn update_ohlcv_core(
        history: &mut Vec<rat_core::OhlcvBar>,
        price: f64,
        now_rfc: &str,
        tick_count: &mut u32,
    ) {
        *tick_count += 1;
        ...
        if history.is_empty() {
            history.push(rat_core::OhlcvBar {
    ...
    async fn run_backtest(
        ...
        while feed.has_next() {
            ...
            for bar in bars {
                ...
                {
                    let ohlcv_bar = rat_core::OhlcvBar {
    ...
    async fn run_validate(
        ...
        use rat_autonomous::self_evolution::SelfEvolutionValidator;

        // Create a fresh orchestrator with shared state (the validator needs owned)
        let mut orch_clone = rat_autonomous::orchestrator_struct::AutonomousOrchestrator::new(
    ...
    pub async fn make_decision(
        ...
        symbol: &str,
        current_price: f64,
    ) -> (Option<rat_autonomous::types::TradeSignal>, String, bool) {
        ...
        if let Some(entry) = cached {
                ...
                .await
            {
                ...
                let direction = if signal.direction == rat_core::TradeDirection::Long {
    ...
    async fn execute_live_trade(
        &self,
        signal: &rat_autonomous::types::TradeSignal,
        market_price: f64,
    ) -> Result<(), String> {
        let registry = self
            .broker_registry
    ...
    async fn signal_from_cache(
        ...
        current_price: f64,
    ) -> Option<rat_autonomous::types::TradeSignal> {
        ...
        Some(rat_autonomous::types::TradeSignal {
