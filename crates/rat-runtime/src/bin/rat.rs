...
enum Command {
    /// Run the trading system (default: paper mode)
    Serve {
    ...
    /// Start the Kronos forecast server
    Start {
...
async fn handle_command(cmd: &Command) -> anyhow::Result<()> {
    ...
    match cmd {
        Command::Start { port } => return handle_start_kronos(*port).await,
    ...
    match cmd {
        ...
        Command::Start { .. } | Command::Download | Command::Serve { .. } => unreachable!(),
    }
    Ok(())
}
