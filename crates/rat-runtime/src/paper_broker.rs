//! Paper broker — wraps rat-core's PaperEngine into a unified execution interface.
//!
...
pub trait ExecutionBroker: Send + Sync {
    ...
    async fn monitor_positions(
        ...
        symbol: &str,
        market_price: f64,
    ) -> Result<Vec<rat_core::paper_engine::ClosedTrade>, Box<dyn std::error::Error + Send + Sync>>
    {
...
/// Paper broker backed by rat-core's `PaperEngine`.
///
...
mod tests {
    use super::*;
    use rat_core::paper_engine::PaperEngineConfig;
    ...
    async fn test_monitor_positions_sl_tp() {
        let config = PaperEngineConfig {
            ...
            slippage_model: rat_core::paper_engine::SlippageModel::None,
            commission_pct: 0.0,
        ...
        assert_eq!(
            ...
            rat_core::paper_engine::CloseReason::StopLoss
        );
    }
