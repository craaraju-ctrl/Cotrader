...
//! (paper vs live) is handled by `rat_core::paper_engine::BrokerRegistry`.
...
//! - `zerodha` — Real trading via Kite Connect v3 (`rat-broker-zerodha` crate)
//! - `alpaca` — US equities/crypto via Alpaca Markets API v2 (`rat-broker-alpaca` crate)
//!
//! ## External plugins
//! Drop a `.toml` file in `~/.rat/plugins/brokers/` with the schema:
//! ```toml
//! [broker]
//! id = "my-broker"
...
use rat_core::paper_engine::{BrokerAdapter, PaperEngineConfig};
...
impl BrokerPluginManager {
    ...
    pub fn new() -> Self {
        let plugins_dir = std::env::var("RAT_PLUGINS_DIR")
            .map(PathBuf::from)
            .unwrap_or_else(|_| {
                let home = std::env::var_os("HOME")
                    .map(PathBuf::from)
                    .unwrap_or_else(|| PathBuf::from("/tmp"));
                home.join(".rat").join("plugins").join("brokers")
            });
        let mut mgr = Self {
            plugins: Vec::new(),
            plugins_dir,
    ...
    /// 3. Saved config files (`~/.rat/{id}.toml`)
    pub async fn instantiate(
        ...
        match plugin.implementation.as_str() {
            "builtin:paper" => {
                let mut balance = merged.get_f64("initial_balance").unwrap_or(100_000.0);
                let default_cfg = PaperEngineConfig::default();
                if default_cfg.base_currency == "INR" && balance <= 100_000.0 {
                    balance *= 83.0;
                }
                ...
                let adapter = rat_core::paper_engine::PaperBroker::new(cfg);
            ...
            "builtin:zerodha" => {
                ...
                // Use the existing rat-broker-zerodha crate
                let broker = rat_broker_zerodha::ZerodhaKiteBroker::new(
                    &api_key,
                    &api_secret,
            ...
            "builtin:upstox" => {
                ...
                let broker = rat_broker_upstox::UpstoxBroker::new(
                    &client_id,
                    &client_secret,
                    &redirect_uri,
                    &access_token,
            ...
            "builtin:angelone" => {
                ...
                let broker = rat_broker_angelone::AngelOneBroker::new(
                    &api_key,
                    &client_id,
                    &pin,
                    totp_secret,
            ...
            "builtin:5paisa" => {
                ...
                let client_code = merged
                    ...
                    .to_string();
                let broker = rat_broker_5paisa::FivePaisaBroker::new(
                    &app_key,
                    &encry_key,
                    &user_id,
                    &client_code,
            ...
            "builtin:alpaca" => {
                ...
                // Use the existing rat-broker-alpaca crate
                use rat_broker_alpaca::AlpacaBroker;
    ...
    fn saved_config_path(&self, id: &str) -> PathBuf {
        let home = std::env::var_os("HOME")
            .map(PathBuf::from)
            .unwrap_or_else(|| PathBuf::from("/tmp"));
        home.join(".rat").join(format!("{}.toml", id))
    }

    /// Save broker configuration to `~/.rat/{id}.toml`.
