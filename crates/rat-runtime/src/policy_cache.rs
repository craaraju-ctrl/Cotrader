...
use rat_autonomous::types::MarketRegime;
...
///
/// Thread-safe via `parking_lot::RwLock`. Persisted to `~/.rat/policy_cache.json`.
...
impl PolicyCache {
    ...
    pub async fn extract_features(&self, symbol: &str) -> MarketFeatures {
        ...
        let (rsi, confluence, volatility_pct, trend_bucket) = match bars {
            Some(ref b) if b.len() >= 14 => {
                let rsi = rat_autonomous::helpers::compute_rsi(b, 14);
                ...
                // Compute trend from recent closes: positive = uptrend, negative = downtrend
                let trend = if b.len() >= 5 {
                    let recent_close = last;
                    let prev_close = b[b.len() - 5].close;
                    if prev_close > 0.0 {
                        ((recent_close - prev_close) / prev_close * 100.0).clamp(-5.0, 5.0)
                    } else {
                        0.0
                    }
                } else {
                ...
                // Bucket trend: 0=strong downtrend, 4=neutral, 8=strong uptrend
                let trend_bucket = ((trend + 5.0) / 1.25).floor().clamp(0.0, 8.0) as i8;
                (rsi, conf, vol, trend_bucket)
            }
            _ => (50.0, 0.5, 0.01, 4),
        };
    ...
    /// Persist the cache to disk (JSON at `~/.rat/policy_cache.json`).
    ...
    fn cache_path() -> PathBuf {
        ...
        home.join(".rat").join("policy_cache.json")
    }
}
...
/// Wrapper to reuse existing ATR computation from rat-autonomous.
mod helpers {
    use rat_core::OhlcvBar;
    pub fn compute_atr(bars: &[OhlcvBar], period: usize) -> f64 {
        rat_autonomous::helpers::compute_atr(bars, period)
    }
}
