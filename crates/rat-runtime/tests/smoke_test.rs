//! Smoke tests for the rat-runtime crate.
//!
//! Run: cargo test -p rat-runtime --test smoke_test
//!
//! These tests verify:
...
use rat_core::{Config, DisciplineRules, MemoryStore};
use rat_runtime::engine::RuntimeEngine;
use rat_runtime::event_bus::{AgentEvent, EventBus};
...
fn setup_test_env(test_name: &str) -> (AutonomousOrchestrator, Vec<String>, String) {
    ...
    let state = rat_autonomous::state::SharedState::new(memory, rules, config, &sqlite_path)
...
async fn test_eventbus_publish_subscribe() {
    ...
    bus.publish(AgentEvent::PriceTick {
        ...
        source: rat_runtime::event_bus::PriceSource::Rest,
    });
...
async fn test_runtime_engine_construction() {
    ...
    engine.event_bus().publish(AgentEvent::PriceTick {
        ...
        volume: 50.0,
        timestamp: chrono::Utc::now(),
        source: rat_runtime::event_bus::PriceSource::Manual,
    });
...
#[tokio::test]
#[ignore] // Requires network access — run manually with: cargo test -p rat-runtime -- --ignored api
async fn test_api_client_fetch_price() {
    ...
    match rat_runtime::api_clients::fetch_binance_price(&client, "BTC").await {
        Ok(price) => {
    ...
    match rat_runtime::api_clients::fetch_yahoo_price(&client, "NIFTY").await {
        Ok(price) => {
    ...
    match rat_runtime::api_clients::fetch_live_bar(&client, "BTC").await {
        Ok(bar) => {
