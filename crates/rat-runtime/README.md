...
The **unified runtime layer** for the rat autonomous agentic trading system.

This crate transforms rat from a batch pipeline into a truly event-driven, multi-mode agentic system. It provides the `RuntimeEngine` that wires together all subsystems — orchestrator, event bus, risk manager, world model, policy cache, active learner, and broker plugins — into a single cohesive runtime.
...
```bash
# Build the runtime
cargo build -p rat-runtime --release

# Run in paper mode (default)
./target/release/rat --mode paper

# Run in validate mode with regret induction
./target/release/rat --mode validate --cycles 100 --induce-regret

# Backtest mode
./target/release/rat --mode backtest --data ./btc_2024.csv --capital 100000

# Live mode (requires --confirm-live)
./target/release/rat --mode live --confirm-live

# Broker configuration
./target/release/rat configure alpaca
./target/release/rat configure zerodha

# List available brokers
./target/release/rat list
```
...
The runtime crate provides the unified `rat` binary (using `clap`):
...
Depends on `rat-core`, `rat-autonomous`, `rat-broker-zerodha`, `rat-broker-alpaca`.
