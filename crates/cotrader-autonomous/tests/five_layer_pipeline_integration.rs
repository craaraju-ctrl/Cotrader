...
use std::fs;
use std::sync::Arc;
use cotrader_autonomous::types::OpenPosition;
...
async fn seed_aggregated_signal(state: &SharedState, conviction: f64) {
    ...
    use cotrader_core::skill_aggregator::AggregatedSignal;
...
async fn test_high_priority_overrides_medium() {
    ...
    *orch.state.market_data.market_regime.write().await =
    ...
    let gate = cotrader_autonomous::hard_rules_gate::HardRulesGate::new(orch.state.clone());
    let gate_result = gate.evaluate("ETH").await;

    assert!(!gate_result.passed, "Gate should block");
    assert_eq!(
        gate_result.highest_failed_priority,
        Some(cotrader_autonomous::types::RulePriority::High),
...
async fn test_deb_layer_with_new_indicators() {
    ...
    *orch.state.market_data.market_regime.write().await =
    ...
    let debate = cotrader_autonomous::debate_layer::DebateLayer::new(orch.state.clone());
...
async fn test_medium_only_block_stops_pipeline() {
    ...
    *orch.state.market_data.market_regime.write().await =
    ...
    assert_eq!(
        gate_result.highest_failed_priority,
        Some(cotrader_autonomous::types::RulePriority::Medium),
