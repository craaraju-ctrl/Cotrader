...
async fn test_debate_agents_without_llm() {
    ...
    let input = cotrader_core::AgentInput::ConfluenceRequest {
        context: cotrader_core::MarketContext {
    ...
    let (action, conf, reason, turns) = cotrader_autonomous::debate::run_debate(
        orch.state.clone(),
        &input,
