// rat_integration.rs
// Integration tests for the Rat agent hierarchy — verifies agent communication,
// data flow through Identifier → Verifier → Executer groups, and shared state integrity.
//
// Run: cargo test -p rat-autonomous --test rat_integration

use chrono::Utc;
use std::fs;
use std::sync::Arc;
use cotrader_autonomous::{
    types::{RiskRecommendation, TradeSignal},
    AutonomousOrchestrator, Executer, Guardian, Identifier, SharedState, Rat, Verifier,
};
...
///
/// Cleans up the shared SQLite `rat_history.db` (opened by SharedState::new)
...
fn setup_test_env(db_name: &str) -> (AutonomousOrchestrator, String) {
    ...
    let state = SharedState::new(memory, rules, config, &sqlite_db_path, paper_broker)
...
fn simulate_outcome_recording(state: &SharedState, symbol: &str, pnl: f64, was_win: bool) {
    ...
    let votes = vec![cotrader_core::SkillVote {
        skill_name: "SentimentAnalyzer".to_string(),
        direction: cotrader_core::SkillDirection::Bullish,
        weight: 0.30,
        confidence: 0.75,
        score: 0.70,
    }];
    ...
    for v in &votes {
        let sp = cotrader_autonomous::episode_store::SkillPerformanceRow {
            id: 0,
    ...
    let closed = cotrader_autonomous::episode_store::ClosedEpisode {
        id: episode_id.clone(),
...
#[tokio::test]
async fn test_rat_hierarchy_integrity() {
    let (orch, db_path) = setup_test_env("hierarchy");
    let rat = orch.rat();

    // Verify the four groups exist with all 16 sub-agents via Arc::ptr_eq
    // Each Rat agent Arc should point to the same instance as the orchestrator's
    assert!(
        Arc::ptr_eq(&orch.scanner, &rat.identifier.scanner),
        "Rat scanner should share orchestrator scanner Arc"
    );
    ...
    // Verify Rat's tree_json is valid
    let tree = Rat::tree_json();
    assert_eq!(tree["name"], "Rat", "Root should be Rat");
    assert_eq!(
    ...
    assert!(
        Arc::ptr_eq(&orch.market_intel, &rat.identifier.market_intel),
    ...
    assert!(
        Arc::ptr_eq(&orch.drawdown, &rat.guardian.drawdown),
        "drawdown Arc should be shared"
    );
    assert!(
        Arc::ptr_eq(&orch.outcome_logger, &rat.guardian.outcome_logger),
...
async fn test_identifier_group() {
    ...
    let result = orch.rat().run_identifier(symbol, 65_000.0, 1).await;
...
async fn test_identifier_without_data() {
    ...
    let result = orch.rat().run_identifier("NIFTY", 24_500.0, 1).await;
...
async fn test_verifier_clean_portfolio() {
    ...
    let result = orch.rat().run_verifier("BTC", 65_000.0, equity, 1).await;
...
async fn test_verifier_with_open_position() {
    ...
    let signal = TradeSignal {
        ...
        position_size: 1.0, // 1 share @ 24,500 = ₹24,500 position value (under paper-mode 95% cap of ₹95,000)
...
async fn test_executer_handles_no_llm() {
    ...
    let result = orch.rat().run_executer("BTC", 65_000.0).await;
...
async fn test_state_sharing_across_groups() {
    ...
    // Read back through Rat Verifier's state reference — should see the modified state
    let equity = get_equity(&orch.state).await;
    let result = rat.run_verifier("NIFTY", 24_500.0, equity, 1).await;
    assert!(result.is_ok());
    let analysis = result.unwrap();
...
async fn test_concurrent_group_access() {
    ...
    let (ident_result, ver_result) = tokio::join!(
        rat.run_identifier("BTC", 65_000.0, 1),
        rat.run_verifier("ETH", 3_500.0, 100_000.0, 2),
    );
...
// ══════════════════════════════════════════════════════════════════════════════
// PORTFOLIO INTEGRATION — verify Rat groups can modify and read portfolio
// ══════════════════════════════════════════════════════════════════════════════

#[tokio::test]
async fn test_portfolio_management_via_rat() {
    let (orch, db_path) = setup_test_env("portfolio_rat");
    let rat = orch.rat();
    ...
    if let Some(signal) = signal_opt {
        ...
        let outcome_ep = cotrader_autonomous::episode_store::ClosedEpisode {
