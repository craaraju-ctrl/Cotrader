...
pub struct HardRulesGate {
    state: SharedState,
    _memory: Option<MemoryIntegration>,
}

impl HardRulesGate {
    pub fn new(state: SharedState) -> Self {
        Self { state, _memory: None }
    }
    ...
    pub fn with_memory(state: SharedState, memory: MemoryIntegration) -> Self {
        Self { state, _memory: Some(memory) }
    }

    /// Simplified rule evaluation — returns default value.
    #[allow(dead_code)]
    fn evaluate_rule_cached(&self, _rule_key: &str, default_value: f64) -> f64 {
        // Simplified: always return default value
        // In production, would use proper caching
        default_value
    }
    ...
    pub async fn evaluate_with_volatility(
        ...
        total_checked += 1;
        let enabled = self.state.portfolio_store.portfolio.read().await.trading_enabled;
        {
            let passed = enabled;
        ...
        total_checked += 1;
        {
            ...
            let red_folder_count = calendar
                .iter()
                .filter(|event| {
                    event.impact == cotrader_core::calendar::EventImpact::High
        ...
        total_checked += 1;
        {
            let is_crypto = cotrader_core::is_crypto_symbol(symbol);
            ...
            if !is_crypto {
                ...
                if rules.respect_session_timing {
                    ...
                    let global_open = cotrader_core::is_in_trading_session(now, &rules);
        ...
        total_checked += 1;
        {
            ...
            let max_trades: u32 = std::env::var("RAT_MAX_DAILY_TRADES")
                .ok()
                .and_then(|v| v.parse().ok())
                .unwrap_or(10);
            let passed = trades < max_trades;
            traces.push(RuleTrace {
                rule_name: "max_daily_trades".to_string(),
                priority: RulePriority::High,
                steps: vec![ReasoningStep {
                    step: "compare_daily_trades".to_string(),
                    description: format!("{} trades today vs {} limit", trades, max_trades),
                    observed: trades as f64,
                    threshold: max_trades as f64,
                    passed,
                }],
                verdict: if passed {
                    "PASS".to_string()
                ...
                confidence: 1.0,
                conclusion: if passed {
                    format!("{} trades today, within {}-trade limit", trades, max_trades)
                } else {
                    format!("{} trades today exceeds {}-trade daily limit", trades, max_trades)
                },
            });
            if !passed {
                failed_rules.push(RuleCheck {
        ...
        total_checked += 1;
        {
            ...
            let (passed, kelly_half, trade_count) = if has_enough_data {
                let kelly = cotrader_core::kelly_criterion_fraction(
                ...
                for i in 0..positions.len() {
                    for j in (i + 1)..positions.len() {
                        ...
                        let corr = if sym_a == sym_b {
                            1.0
                        } else if cotrader_core::is_crypto_symbol(sym_a)
                            && cotrader_core::is_crypto_symbol(sym_b)
                        {
        ...
        // ═══ RULES 18–22: added by risk-rule upgrade (see docs/RULES_AUDIT.md) ═══
        // Modeled on SEC 15c3-5 pre-trade controls: erroneous-order prevention,
        // data-integrity checks, and capital/concentration thresholds.

        // 18. Black swan detector — >5% single-bar move = flash-crash protection.
        //     Never enter during a dislocation; spreads/slippage are unmodeled there.
        total_checked += 1;
        {
            let bars = snapshot.bars();
            let (change, prev_close) = if bars.len() >= 2 {
                let current = bars[bars.len() - 1].close;
                let prev = bars[bars.len() - 2].close;
                if prev > 0.0 {
                    ((current - prev).abs() / prev, prev)
                } else {
                    (0.0, 0.0)
                }
            } else {
            ...
            let passed = change <= 0.05;
            traces.push(RuleTrace {
                rule_name: "black_swan_detector".to_string(),
                priority: RulePriority::Critical,
                steps: vec![ReasoningStep {
                    step: "compare_bar_move".to_string(),
                    description: format!(
                        "Last bar move {:.2}% vs 5.0% flash-crash threshold (prev close {:.4})",
                        change * 100.0, prev_close
                    ),
                    observed: change,
                    threshold: 0.05,
                    passed,
                ...
                confidence: 1.0,
                conclusion: if passed {
                    format!("No dislocation — last bar moved {:.2}%", change * 100.0)
                } else {
                    format!(
                        "BLACK SWAN: {:.2}% single-bar move exceeds 5% — no entries during dislocation",
                        change * 100.0
                    )
                },
            });
            if !passed {
                failed_rules.push(RuleCheck {
                    ...
                    rule_name: "black_swan_detector".to_string(),
                    priority: RulePriority::Critical,
                    reason: format!("{:.2}% single-bar move exceeds 5% flash-crash threshold", change * 100.0),
                });
                highest_blocking_priority = Some(RulePriority::Critical);
            }
        }

        // 19. Stale market data — a rule is only as good as its inputs. If the
        //     newest bar is older than 2 hours the feed is dead/degraded; trading
        //     on stale prices is trading blind. Unparseable timestamps pass
        //     (informational) to avoid false halts.
        total_checked += 1;
        {
            let bars = snapshot.bars();
            let age_minutes: Option<f64> = bars.last().and_then(|b| {
                chrono::DateTime::parse_from_rfc3339(&b.timestamp)
                    .ok()
                    .map(|ts| {
                        (Utc::now() - ts.with_timezone(&chrono::Utc)).num_seconds() as f64 / 60.0
                    })
            });
            let max_age_min = 120.0;
            let (observed, passed) = match age_minutes {
                Some(age) => (age, age <= max_age_min),
                None => (0.0, true),
            };
            traces.push(RuleTrace {
                rule_name: "stale_market_data".to_string(),
                priority: RulePriority::Critical,
                steps: vec![ReasoningStep {
                    step: "check_bar_age".to_string(),
                    description: match age_minutes {
                        Some(age) => format!(
                            "Newest {} bar is {:.0} min old (max {:.0} min)",
                            symbol, age, max_age_min
                        ),
                        None => "Bar timestamp unparseable — check skipped".to_string(),
                    },
                    observed,
                    threshold: max_age_min,
                    passed,
                }],
                verdict: if passed { "PASS".to_string() } else { "BLOCK".to_string() },
                confidence: if age_minutes.is_some() { 1.0 } else { 0.3 },
                conclusion: if passed {
                    "Market data fresh".to_string()
                } else {
                    format!(
                        "STALE DATA: newest bar {:.0} min old — feed degraded, trading blind",
                        observed
                    )
                },
            });
            if !passed {
                failed_rules.push(RuleCheck {
                    ...
                    rule_name: "stale_market_data".to_string(),
                    priority: RulePriority::Critical,
                    reason: format!("Newest bar {:.0} min old (max {:.0}) — stale feed", observed, max_age_min),
                });
                highest_blocking_priority = Some(RulePriority::Critical);
            }
        }

        // 20. Net directional exposure — observed live: 9/9 positions LONG crypto,
        //     which is one correlated bet, not nine trades. Block new entries when
        //     long exposure exceeds 80% of equity.
        total_checked += 1;
        {
            let portfolio = self.state.portfolio_store.portfolio.read().await;
            let eq = portfolio.total_equity.max(1.0);
            let long_value: f64 = portfolio
                .open_positions
                .iter()
                .filter(|p| p.direction == cotrader_core::TradeDirection::Long)
                .map(|p| p.quantity * p.current_price)
                .sum();
            let long_pct = long_value / eq;
            let max_directional = 0.80;
            let passed = long_pct <= max_directional;
            traces.push(RuleTrace {
                rule_name: "net_directional_exposure".to_string(),
                priority: RulePriority::High,
                steps: vec![ReasoningStep {
                    step: "compare_long_exposure".to_string(),
                    description: format!(
                        "Long exposure {:.1}% of equity vs {:.0}% cap ({} open positions)",
                        long_pct * 100.0,
                        max_directional * 100.0,
                        portfolio.open_positions.len()
                    ),
                    observed: long_pct,
                    threshold: max_directional,
                    passed,
                }],
                verdict: if passed { "PASS".to_string() } else { "BLOCK".to_string() },
                confidence: 0.95,
                conclusion: if passed {
                    format!("Directional exposure OK ({:.1}% long)", long_pct * 100.0)
                } else {
                    format!(
                        "One-sided book: {:.1}% of equity long — correlated crypto longs are ONE bet",
                        long_pct * 100.0
                    )
                },
            });
            if !passed {
                failed_rules.push(RuleCheck {
                    ...
                    priority: RulePriority::High,
                    reason: format!(
                        "Long exposure {:.1}% of equity exceeds {:.0}% cap",
                        long_pct * 100.0, max_directional * 100.0
                    ),
                });
                if highest_blocking_priority.is_none()
                    || highest_blocking_priority < Some(RulePriority::High)
        ...
        // 21. Exposure concentration — no single symbol may exceed 20% of equity.
        total_checked += 1;
        {
            let portfolio = self.state.portfolio_store.portfolio.read().await;
            let eq = portfolio.total_equity.max(1.0);
            ...
            let max_concentration = 0.20;
            let passed = sym_pct <= max_concentration;
            traces.push(RuleTrace {
                rule_name: "exposure_concentration".to_string(),
                priority: RulePriority::High,
                steps: vec![ReasoningStep {
                    step: "compare_symbol_exposure".to_string(),
                    description: format!(
                        "{} exposure {:.1}% of equity vs {:.0}% cap",
                        symbol, sym_pct * 100.0, max_concentration * 100.0
                    ),
                    observed: sym_pct,
                ...
                confidence: 0.95,
                conclusion: if passed {
                    format!("{} concentration OK ({:.1}%)", symbol, sym_pct * 100.0)
                } else {
                    format!(
                        "{} already {:.1}% of equity — exceeds 20% single-symbol cap",
                        symbol, sym_pct * 100.0
                    )
                },
            });
            if !passed {
                failed_rules.push(RuleCheck {
                    ...
                    priority: RulePriority::High,
                    reason: format!(
                        "{} exposure {:.1}% of equity exceeds 20% cap",
                        symbol, sym_pct * 100.0
                    ),
                });
                if highest_blocking_priority.is_none()
        ...
        // 22. Liquidity floor — average bar notional (volume × close) over the
        //     last 20 bars must exceed $50k or exits will suffer brutal slippage.
        total_checked += 1;
        {
            let bars = snapshot.bars();
            let window = bars.len().min(20);
            let avg_notional = if window > 0 {
            ...
            traces.push(RuleTrace {
                ...
                priority: RulePriority::High,
                steps: vec![ReasoningStep {
                    step: "compare_avg_notional".to_string(),
                    description: format!(
                        "Avg bar notional ${:.0} over {} bars vs ${:.0} floor (volume data: {})",
                        avg_notional, window, min_notional, has_volume
                    ),
                    observed: avg_notional,
                    threshold: min_notional,
                    passed,
                }],
                verdict: if passed { "PASS".to_string() } else { "BLOCK".to_string() },
                confidence: if has_volume { 0.9 } else { 0.3 },
                conclusion: if !has_volume {
                    "No volume data — liquidity check skipped".to_string()
                } else if passed {
                    format!("Liquidity OK (${:.0}/bar avg)", avg_notional)
                } else {
                    format!(
                        "ILLIQUID: ${:.0}/bar avg notional < ${:.0} floor — exit slippage risk",
                        avg_notional, min_notional
                    )
                },
            });
            if !passed {
                failed_rules.push(RuleCheck {
                    ...
                    priority: RulePriority::High,
                    reason: format!(
                        "Avg bar notional ${:.0} below ${:.0} liquidity floor",
                        avg_notional, min_notional
                    ),
                });
                if highest_blocking_priority.is_none()
...
mod tests {
    ...
    async fn setup_state_max_trades() -> SharedState {
        ...
        {
            let mut portfolio = state.portfolio_store.portfolio.write().await;
            portfolio.total_trades_today = 11; // Exceeds limit of 10
        }
        state
    }
    ...
    async fn setup_state_bear_regime_low_confluence() -> SharedState {
        let state = setup_state().await;
        {
            *state.market_data.market_regime.write().await = Some(MarketRegime::TrendingBear);
            let agg = cotrader_core::skill_aggregator::AggregatedSignal {
                net_signal: -0.3,
                bullish_strength: 0.2,
                bearish_strength: 0.6,
                conviction: 0.25, // below bear threshold 0.30
                consensus: Some(cotrader_core::agent::SkillDirection::Bearish),
        ...
        let state = setup_state().await;
        {
            *state.market_data.market_regime.write().await = Some(MarketRegime::TrendingBull);
            ...
            let agg = cotrader_core::skill_aggregator::AggregatedSignal {
                ...
                consensus: Some(cotrader_core::agent::SkillDirection::Bullish),
    ...
    async fn test_all_rules_pass() {
        ...
        assert_eq!(result.total_rules_checked, 22, "Should check all 22 rules");
    }
        ...
        let state = setup_state().await;
        {
            *state.market_data.market_regime.write().await = Some(MarketRegime::Ranging);
            let agg = cotrader_core::skill_aggregator::AggregatedSignal {
                ...
                consensus: Some(cotrader_core::agent::SkillDirection::Neutral),
                participating_count: 3,
                bullish_count: 1,
                bearish_count: 2,
                neutral_count: 0,
            ...
            *state.agent_memory.last_aggregated_signal.write().await = Some(agg);
            ...
            let bar = cotrader_core::OhlcvBar {
    ...
    async fn test_all_17_rules_checked() {
        ...
        let gate = HardRulesGate::new(state);
        let result = gate.evaluate("BTC").await;
        // 17 original + 5 added (black_swan, stale_data, net_exposure, concentration, liquidity) = 22
        assert_eq!(
            result.total_rules_checked, 22,
            "Should check all 22 rules regardless of pass/fail"
        );
    }
    ...
    async fn test_traces_populated() {
        ...
        assert_eq!(cor.traces.len(), 22, "Should have 22 traces (one per rule)");
    ...
    async fn test_traces_on_block() {
        ...
        assert_eq!(
            cor.traces.len(),
            22,
            "Should have 22 traces even when blocked"
        );
