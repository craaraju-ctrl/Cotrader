...
/// A neurosymbolic rule discovered from trade outcomes.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct NeurosymbolicRuleRow {
    pub id: String,
    pub name: String,
...
impl EpisodeStore {
            ...
            match Connection::open(path) {
                Ok(conn) => {
                    ...
                    conn.execute_batch(
                        ...
                        -- Neurosymbolic learned rules (discovered from trade outcomes)
                        ...
                        CREATE INDEX IF NOT EXISTS idx_ns_rules_active ON neurosymbolic_rules(active);
                        CREATE INDEX IF NOT EXISTS idx_ns_rules_source ON neurosymbolic_rules(source);
                    ",
                    )?;
                ...
                Err(e) => {
                    ...
                    return Err(last_err.unwrap_or_else(|| {
                        rusqlite::Error::SqliteFailure(
                            ...
                            Some("Failed to open database after retries".into()),
                        )
                    }));
                }
            }
    ...
    pub fn insert_neurosymbolic_rule(&self, rule: &NeurosymbolicRuleRow) -> Result<(), rusqlite::Error> {
        ...
        conn.execute(
            ...
            params![
                rule.id, rule.name, rule.condition, rule.action, rule.priority,
                rule.source, rule.confidence, rule.times_applied, rule.times_correct,
                rule.created_at, rule.last_applied, rule.active as i64,
            ],
        )?;
        Ok(())
    }
    ...
    pub fn load_active_neurosymbolic_rules(&self) -> Result<Vec<NeurosymbolicRuleRow>, rusqlite::Error> {
        ...
        let mut stmt = conn.prepare(
            "SELECT id, name, condition, action, priority, source, confidence,
                    times_applied, times_correct, created_at, last_applied, active
             FROM neurosymbolic_rules WHERE active = 1
             ORDER BY confidence DESC, times_applied DESC"
        )?;

        let rules = stmt.query_map([], |row| {
            Ok(NeurosymbolicRuleRow {
                id: row.get(0)?,
                name: row.get(1)?,
                condition: row.get(2)?,
                action: row.get(3)?,
    ...
    /// Update rule usage stats (times_applied, times_correct, last_applied).
    pub fn update_neurosymbolic_rule_stats(
        &self,
        rule_id: &str,
        applied: bool,
        correct: bool,
            ...
            conn.execute(
                "UPDATE neurosymbolic_rules
                 SET times_applied = times_applied + 1,
                     ...
                     last_applied = ?2
                 WHERE id = ?3",
                params![if correct { 1 } else { 0 }, chrono::Utc::now().to_rfc3339(), rule_id],
            )?;
        }
        Ok(())
    }
