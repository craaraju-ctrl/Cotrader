...
use cotrader_core::{AgentInput, AgentOutput, OhlcvBar};
...
impl MarketMetricsMeter {
    ...
    pub async fn compute(&self, symbol: &str, current_price: f64) -> MetricsSnapshot {
        ...
        // Delegate to the stateless compute_on_bars to avoid code duplication
        let mut snap = Self::compute_on_bars(&bars, symbol, current_price, "local_ohlcv");
