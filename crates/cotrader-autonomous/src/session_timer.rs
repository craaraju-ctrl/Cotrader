...
use chrono::Utc;
use std::error::Error;
use cotrader_core::{Agent, AgentInput, AgentOutput, AgentTier, DisciplineCheck};
...
impl Agent for SessionTimerAgent {
    ...
    async fn run(
        ...
        if let Some(AgentInput::RiskRequest { context }) = &input {
            is_crypto = cotrader_core::is_crypto_symbol(&context.symbol);
        } else if let Some(AgentInput::ConfluenceRequest { context }) = &input {
            is_crypto = cotrader_core::is_crypto_symbol(&context.symbol);
        } else {
            // No symbol context — check if primary trading is crypto
            // (the system trades crypto 24/7, weekends included)
            is_crypto = true;
        }

        let (active, status) = if is_crypto {
            (true, "ACTIVE | Crypto 24/7 Session".to_string())
        } else {
            self.is_nse_session_active()
        };
