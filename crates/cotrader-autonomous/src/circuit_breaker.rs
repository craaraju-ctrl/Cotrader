...
pub struct CircuitBreaker {
    ...
    _episode_store: Option<Arc<EpisodeStore>>,
    /// Set when a halt fires within `min_halt_interval_secs` of the previous
    /// one — extends the auto-resume gate (D2 fix: replaces halt suppression).
    rapid_rehalt: RwLock<bool>,
}
...
impl CircuitBreaker {
    ...
    pub fn with_episode_store(config: CircuitBreakerConfig, episode_store: Arc<EpisodeStore>) -> Self {
        Self {
            ...
            connection_drop_count: RwLock::new(0),
            recent_events: RwLock::new(Vec::new()),
            _episode_store: Some(episode_store),
            rapid_rehalt: RwLock::new(false),
        }
    }
    ...
    pub async fn is_trading_allowed(&self) -> bool {
        // Global coordination (D4 fix): if any OTHER subsystem announced a
        // halt (memory breaker open, RiskManager hard stop), trading is not
        // allowed regardless of this breaker's local state.
        if cotrader_core::breaker_net::is_halted_by_other(cotrader_core::breaker_net::BreakerId::Trading) {
            return false;
        }
        ...
        match *state {
            ...
            BreakerState::Recovery => {
                ...
                if let Some(halted_at) = halted_at_val {
                    ...
                    if elapsed >= self.config.cool_down_secs.max(self.resume_gate_secs().await) {
                        *state = BreakerState::Armed;
                        let mut reason = self.halt_reason.write().await;
                        *reason = None;
                        let mut halted = self.halted_at.write().await;
                        *halted = None;
                        cotrader_core::breaker_net::resume(cotrader_core::breaker_net::BreakerId::Trading);
                        println!(
    ...
    /// Resume-gate extension (replaces the removed halt-suppression logic):
    /// if this halt came shortly after a previous one (unstable environment),
    /// require `min_halt_interval_secs` of quiet before auto-resume instead
    /// of the plain cool-down. Faults are never ignored; recovery just slows.
    async fn resume_gate_secs(&self) -> u64 {
        if *self.rapid_rehalt.read().await {
            self.config.min_halt_interval_secs
        } else {
    ...
    pub async fn manual_reset(&self) {
        ...
        *drops = 0;
        ...
        let mut peak = self.daily_peak_equity.write().await;
        *peak = None;
        *self.rapid_rehalt.write().await = false;

        // Clear the global latch too — manual reset is the audited resume path.
        cotrader_core::breaker_net::resume(cotrader_core::breaker_net::BreakerId::Trading);
    ...
    async fn trigger_halt(&self, reason: HaltReason) {
        ...
        // FIXED (fault-isolation audit, D2): a fresh fault while Armed must
        // ALWAYS halt. Previously `min_halt_interval_secs` caused trigger_halt()
        // to RETURN WITHOUT HALTING for any new fault within 10 minutes of the
        // last halt — confirmed live: a 5% slippage fault right after an
        // auto-resume left the breaker Armed and trading enabled. The interval
        // now only prevents rapid halt/resume *cycling* via the resume gate in
        // is_trading_allowed(); it never ignores a live fault.

        *state = BreakerState::Halted;
        let mut reason_lock = self.halt_reason.write().await;
        *reason_lock = Some(reason.clone());
        // Rapid re-halt? (read the PREVIOUS halt time before overwriting)
        {
            let prev = *self.last_halt_at.read().await;
            let is_rapid = prev.is_some_and(|p| {
                ((now - p).num_seconds().unsigned_abs()) < self.config.min_halt_interval_secs
            });
            *self.rapid_rehalt.write().await = is_rapid;
        }
        let mut halted_at = self.halted_at.write().await;
        *halted_at = Some(now);
        ...
        *last_halt = Some(now);
        ...
        // Propagate to the global breaker coordination channel (D4 fix): the
        // memory layer and RiskManager immediately see this halt.
        cotrader_core::breaker_net::announce_halt(
            cotrader_core::breaker_net::BreakerId::Trading,
            &reason.to_string(),
        );
