...
use cotrader_core::paper_engine::{OrderRequest, OrderStatus, OrderType, TradingMode};
use cotrader_core::{Agent, AgentInput, AgentOutput, AgentTier, TradeOutcome, TradingEpisode};
...
/// Validation-only: when `RAT_INDUCE_REGRET_SL_PCT` is set to a positive
/// number, paper trades have their stop tightened to that percentage from entry
/// so they reliably hit SL → losses → high regret. Used by the self-evolution
/// validator's `--induce` mode to exercise the meta-control adaptation loop.
/// Returns `None` (no effect) in normal operation.
fn induced_regret_sl_pct() -> Option<f64> {
    std::env::var("RAT_INDUCE_REGRET_SL_PCT")
        .ok()
...
impl ExecutionCoordinatorAgent {
    ...
    fn validate_signal_geometry(signal: &TradeSignal) -> Result<(), String> {
        ...
        // FIX: Minimum risk distance — prevent degenerate levels for penny assets.
        // For ADA at $0.19, SL at $0.1887 is only 0.7% away — one candle can hit it.
        // Enforce at least 0.5% risk distance to ensure meaningful stop placement.
        let min_risk_pct = 0.005; // 0.5%
        let risk_pct = match signal.direction {
            cotrader_core::TradeDirection::Long => {
                (signal.entry_price - signal.stop_loss).abs() / signal.entry_price
            }
            cotrader_core::TradeDirection::Short => {
                (signal.stop_loss - signal.entry_price).abs() / signal.entry_price
            }
        };
        if risk_pct < min_risk_pct {
            return Err(format!(
                "Stop loss too close to entry: {:.3}% risk < {:.1}% minimum (degenerate levels for ${:.2} asset)",
                risk_pct * 100.0, min_risk_pct * 100.0, signal.entry_price
            ));
        }

        // R:R floor (final safety net): 1% capital risk must chase ≥2% capital
        // reward. Signal sources enforce 2.0; 1.5 tolerance here absorbs
        // slippage/rounding so we reject only genuinely bad geometry.
        let reward_pct = match signal.direction {
            cotrader_core::TradeDirection::Long => {
        ...
        if reward_pct < risk_pct * 1.5 {
            return Err(format!(
                "Risk:Reward too low: {:.2}:1 < 1.5:1 floor (risk {:.2}%, reward {:.2}%) — 1R risk must target ≥2R",
                reward_pct / risk_pct.max(1e-12),
                risk_pct * 100.0,
                reward_pct * 100.0
            ));
        }
    ...
    async fn compliance_check(
        ...
        let compliance_url = match std::env::var("COMPLIANCE_URL") {
            ...
            _ => {
                ...
                println!("[Compliance] ℹ COMPLIANCE_URL not set — compliance checks disabled. Set COMPLIANCE_URL to a rat-compliance gateway address to enable.");
                return Ok(());
            }
        };
        ...
        let (
            ...
            // equity (see portfolio_manager). Dividing by equity again produced
            // ~1e-7 values, so downstream consumers saw zero drawdown forever.
        ...
        let proposal = ComplianceProposal {
            ...
            direction: if signal.direction == cotrader_core::TradeDirection::Long {
                "BUY"
            } else {
                "SELL"
            }
    ...
    pub async fn execute_paper_trade_with_chain(
        ...
        let direction_str = if signal.direction == cotrader_core::TradeDirection::Long {
            "BUY"
        } else {
            "SELL"
        };
        ...
        // ── RiskGuardian firewall — enforce hard safety limits ──────────────
        {
            let guardian = crate::risk_guardian::RiskGuardian::new(
                crate::risk_guardian::RiskGuardianConfig::default_fallback()
            );
            let portfolio = self.state.portfolio_store.portfolio.read().await;
            // FIXED (verification pass): max_drawdown_today is ALREADY a fraction
            // of equity — dividing by equity again gave the RiskGuardian a ~1e-7
            // drawdown, so its firewall could never trip. Second occurrence of
            // the same unit bug (first was in the compliance snapshot block).
            let dd_pct = portfolio.max_drawdown_today;
            let context = crate::risk_guardian::GuardianPortfolioContext {
                current_drawdown_pct: dd_pct,
                total_equity: portfolio.total_equity,
            };
            let proposed = crate::risk_guardian::ProposedTrade {
                symbol: signal.symbol.clone(),
                entry_price: signal.entry_price,
            ...
            if let Err(reason) = guardian.intercept_and_validate(&proposed, &context) {
                println!("[RiskGuardian] ❌ BLOCKED: {}", reason);
                self.push_execution_cot(
                    cot_chain_id,
                    signal,
                    "RiskGuardian",
                    &format!(
                        "RiskGuardian validate {} {} @ {:.2}",
                        signal.symbol, direction_str, signal.entry_price
                    ),
                    "BLOCKED",
                    &reason,
        ...
        let effective_entry = match signal.direction {
            cotrader_core::TradeDirection::Long => signal.entry_price * (1.0 + slippage_pct / 100.0),
            cotrader_core::TradeDirection::Short => signal.entry_price * (1.0 - slippage_pct / 100.0),
        };
        ...
        // Induced-regret override (validation only — see `induced_regret_sl_pct`).
        let effective_sl = match induced_regret_sl_pct() {
            Some(pct) => {
                let tight = match signal.direction {
                    cotrader_core::TradeDirection::Long => effective_entry * (1.0 - pct / 100.0),
                    cotrader_core::TradeDirection::Short => effective_entry * (1.0 + pct / 100.0),
                };
                println!(
                    "[ExecutionCoordinator] ⚠ INDUCED REGRET: tightening SL {:.2} → {:.2} ({:.2}% from entry)",
                    effective_sl, tight, pct
                );
                tight
            }
            None => effective_sl,
        ...
        // R:R discipline after slippage: entry/SL shifted, so re-extend TP if the
        // post-slippage geometry fell below 2:1 (1% risk → ≥2% reward contract).
        let (effective_tp, _rr_after_slippage) = crate::helpers::enforce_min_risk_reward(
            signal.direction,
            effective_entry,
            effective_sl,
            effective_tp,
        ...
        println!(
            ...
            signal.symbol,
            if signal.direction == cotrader_core::TradeDirection::Long {
                "BUY"
            } else {
                "SELL"
            },
        ...
        // Paper fill by the LOCAL paper engine: real market prices, real
        // volatility-adaptive slippage, real portfolio accounting — the fill
        ...
        if let Err(e) = self.state.agent_memory.memory.store_decision(
            &format!("execution/{}/{}", signal.symbol, Utc::now().timestamp()),
            &exec_log,
        ) {
            eprintln!("[ExecutionCoordinator] ⚠ Failed to log execution to memory: {}", e);
        }
                    ...
                    if let Ok(mut episode) = serde_json::from_str::<TradingEpisode>(&json) {
                        ...
                        if episode.outcome.is_none() {
                            ...
                            if let Ok(updated_json) = serde_json::to_string(&episode) {
                                if let Err(e) = self.state.agent_memory.memory.store_episode(&episode_id, &updated_json) {
                                    eprintln!("[OutcomeUpdate] ⚠ Failed to store episode {}: {}", episode_id, e);
                                }
                                println!("[OutcomeUpdate] 📝 Episode {} updated with outcome: {} {:.2} (P&L: ${:.2})",
                                    episode_id, exit_reason, exit_price, pnl);

                                // Store outcome in vector memory for future semantic recall
                                let summary = format!(
    ...
    pub async fn refresh_position_prices(&self) {
        ...
        let open_value: f64 = portfolio
            ...
            .map(|p| match p.direction {
                cotrader_core::TradeDirection::Long => p.quantity * p.current_price,
                cotrader_core::TradeDirection::Short => {
        ...
        // FIXED (rules audit): track UNREALIZED drawdown on every price refresh —
        // this loop runs constantly, so the 2% daily limit can now fire while
        // positions are open instead of only after they close.
        let unrealized: f64 = portfolio
            .open_positions
            .iter()
            .map(|p| p.unrealized_pnl)
            .sum();
        let floating_pnl = portfolio.daily_pnl + unrealized;
        if floating_pnl < 0.0 && portfolio.total_equity > 0.0 {
            let dd = floating_pnl.abs() / portfolio.total_equity;
            if dd > portfolio.max_drawdown_today {
                portfolio.max_drawdown_today = dd;
            }
        }
        let rules = self.state.rule_engine.rules.read().await;
        if portfolio.trading_enabled && portfolio.max_drawdown_today >= rules.max_daily_drawdown {
            portfolio.trading_enabled = false;
            println!(
                "[ExecutionCoordinator] 🛑 TRADING HALTED — floating drawdown {:.2}% ≥ {:.2}% daily limit",
    ...
    async fn check_and_exit_positions(&self) -> Result<Vec<String>, Box<dyn Error + Send + Sync>> {
                ...
                if !active_on_broker {
                    ...
                    match pm.close_position(&pos.symbol, exit_price).await {
                        Ok(pnl) => {
                            ...
                            let closed = cotrader_core::ClosedTrade {
                                ...
                                close_reason: match exit_reason {
                                    "stop_loss" => cotrader_core::CloseReason::StopLoss,
                                    "take_profit" => cotrader_core::CloseReason::TakeProfit,
                                    _ => cotrader_core::CloseReason::Manual,
                                },
                            ...
                            if let Err(e) = self.state.agent_memory.memory.store_decision(
                                ...
                                eprintln!("[ExecutionCoordinator] ⚠ Failed to store closed trade: {}", e);
                            }
        ...
        for pos in &positions_snapshot {
            ...
            // ═══ TIME-BASED EXIT ═══════════════════════════════════════════
            // If neither SL nor TP has been hit after the configured time,
            // close the position at current price to free up locked capital.
            // Default: 8 hours (28800s). Override with RAT_TIME_EXIT_SECS env var.
            let time_exit_secs: u64 = std::env::var("RAT_TIME_EXIT_SECS")
                .ok()
                .and_then(|v| v.parse().ok())
                .unwrap_or(28800); // 8 hours default
            let elapsed_secs = (Utc::now() - pos.entry_time).num_seconds();
            let time_exit = !stop_hit && !tp_hit && elapsed_secs > time_exit_secs as i64;
                ...
                match pm.close_position(&pos.symbol, pos.stop_loss).await {
                    Ok(pnl) => {
                        ...
                        let closed = cotrader_core::ClosedTrade {
                            ...
                            realized_pnl: pnl,
                            realized_pnl_pct: (pnl / (pos.entry_price * pos.quantity)).abs(),
                            close_reason: cotrader_core::CloseReason::StopLoss,
                            opened_at: pos.entry_time,
                            closed_at: Utc::now(),
                            duration_secs: (Utc::now() - pos.entry_time).num_seconds(),
                ...
                match pm.close_position(&pos.symbol, pos.take_profit).await {
                    Ok(pnl) => {
                        ...
                        let closed = cotrader_core::ClosedTrade {
                            ...
                            realized_pnl: pnl,
                            realized_pnl_pct: (pnl / (pos.entry_price * pos.quantity)).abs(),
                            close_reason: cotrader_core::CloseReason::TakeProfit,
                            opened_at: pos.entry_time,
                            closed_at: Utc::now(),
                            duration_secs: (Utc::now() - pos.entry_time).num_seconds(),
                            strategy: Some("debate_driven".to_string()),
                ...
                match pm.close_position(&pos.symbol, current_price).await {
                    Ok(pnl) => {
                        ...
                        let closed = cotrader_core::ClosedTrade {
                            ...
                            realized_pnl: pnl,
                            realized_pnl_pct: (pnl / (pos.entry_price * pos.quantity)).abs(),
                            close_reason: cotrader_core::CloseReason::Manual,
                            opened_at: pos.entry_time,
                            closed_at: Utc::now(),
                            duration_secs: (Utc::now() - pos.entry_time).num_seconds(),
                            strategy: Some("debate_driven".to_string()),
        ...
        match input {
            Some(AgentInput::ConfluenceRequest { context }) => {
                let signal = TradeSignal {
                    ...
                    direction: cotrader_core::TradeDirection::Long,
...
async fn build_pre_trade_snapshot(
    ...
    let direction_str = match signal.direction {
        cotrader_core::TradeDirection::Long => "BUY",
        cotrader_core::TradeDirection::Short => "SELL",
    };
...
// =====================================================================
// Compliance Gateway Types — mirror of rat-compliance's API types
// =====================================================================
...
mod compliance_tests {
    ...
    // These are tested at the crate level in rat-compliance
}
