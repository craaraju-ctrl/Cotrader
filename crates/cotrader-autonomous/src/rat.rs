// rat.rs
// RAT Agent Hierarchy — logical grouping of sub-agents under four managers.
//
//  Rat (main orchestrator)
...
// ─────────────────────────────────────────────────────────────────────────────
// RAT — the top-level orchestration view
// ─────────────────────────────────────────────────────────────────────────────

/// Rat is the top-level agent hierarchy wrapper.
///
...
pub struct Rat {
    ...
    /// 8 compressed agents with real reasoning (new architecture)
    pub agents: crate::agents::RatAgents,
}

impl Rat {
    /// Build a `Rat` view from an existing `AutonomousOrchestrator`.
    ///
    ...
    /// Print the full Rat agent tree to stdout.
    pub fn print_tree() {
        println!(
            "\nRat\n\
             ├── {}\n\
    ...
    pub async fn run_identifier(
        ...
        price: f64,
        chain_id: u64,
    ) -> Result<(bool, f64, cotrader_core::PivotLevels), Box<dyn Error + Send + Sync>> {
        println!(
            "[Rat::Identifier] Scanning market for {} @ {:.2}",
            symbol, price
        );
        ...
        let pivot_result = self
            .identifier
            .pivot_calc
            .run(Some(cotrader_core::AgentInput::PivotRequest {
                high: price * 1.01,
                low: price * 0.99,
                close: price,
            }))
        ...
        let context = cotrader_core::MarketContext {
            ...
            trend_direction: Some(cotrader_core::TrendDirection::Neutral),
        };
        let conf_result = self.identifier.confluence.run(Some(cotrader_core::AgentInput::ConfluenceRequest { context: context.clone() })).await;
        self.identifier
            .confluence
            .state
            .add_cot_step(
        ...
        let pat_result = self.identifier.pattern_retriever.run(Some(cotrader_core::AgentInput::ConfluenceRequest { context })).await;
        self.identifier
            .pattern_retriever
            .state
            .add_cot_step(
        ...
        let mock_context = cotrader_core::MarketContext {
            symbol: symbol.to_string(),
            current_price: 0.0,
            high: 0.0,
            low: 0.0,
        ...
        let session_ok = self.identifier.session_timer.run(Some(cotrader_core::AgentInput::RiskRequest { context: mock_context })).await.is_ok();
        self.identifier
            .session_timer
            .state
            .add_cot_step(
        ...
        if !discipline_ok {
            println!(
                "[Rat::Identifier] ⚠ Discipline checks failed (session: {}, red_folder: {})",
                session_ok, red_ok
            );
        }

        println!(
            "[Rat::Identifier] ✅ Analysis complete — Confluence: {:.1}%, Discipline: {}",
            confluence * 100.0,
    ...
    pub async fn run_verifier(
        ...
        println!(
            "[Rat::Verifier] Validating risk for {} @ {:.2} (equity: {:.2})",
            symbol, price, equity
        );
        ...
        let analysis = self
            .verifier
            .risk_psych
            .analyze_risk(&cotrader_core::MarketContext {
                symbol: symbol.to_string(),
                current_price: price,
                high: price * 1.01,
                low: price * 0.99,
        ...
        println!(
            "[Rat::Verifier] ✅ Risk check — Recommendation: {:?}, Heat: {:.1}%",
            analysis.recommendation,
            analysis.portfolio_heat * 100.0
        );
        Ok(analysis)
    ...
    pub async fn run_executer_with_aggregation(
        ...
        println!(
            "[Rat::Executer] Agent making fully autonomous decision for {} @ current {:.2} (using aggregated skills consensus)",
        ...
        match &signal_opt {
            Some(sig) => {
                println!(
                    "[Rat::Executer] ✅ AGENT decided {:?} {} @ entry={:.2} SL={:.2} TP={:.2} (conf: {:.1}%)",
                    sig.direction,
                    symbol,
                    sig.entry_price,
                    sig.stop_loss,
                ...
                println!("[Rat::Executer] ✅ {}", exec_result);
