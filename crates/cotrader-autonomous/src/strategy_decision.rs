...
use crate::types::{AgentDecision, DecisionVerdict, MarketRegime, TradeSignal};
use chrono::Utc;
use std::error::Error;
use cotrader_core::{
...
use cotrader_core::messages::LLMRequest;
...
impl StrategyDecisionAgent {
    ...
    pub async fn evaluate_market_and_discover_price(
        ...
        if current_price < system_discovered_entry_price {
            ...
            // Kelly Criterion with ML-predicted win probability:
            // f = p - (q / r) where p = ML-predicted win_prob, r = 2.0
            let fallback_win_prob = 0.55;
            // Build minimal features for ML prediction (this method doesn't have full indicator context)
            let empty_bars: Vec<cotrader_core::OhlcvBar> = Vec::new();
            let ml_features = self.state.ml_engine.feature_store().build_features(
                ...
                &empty_bars, Some(MarketRegime::Ranging), 0.02, 0, 0.0,
            );
            let (win_prob, _ml_source) = self.state.ml_engine.predict_win_probability(
                &ml_features, fallback_win_prob,
            ).await;
            let risk_reward = 2.0;
            ...
            let signal = TradeSignal {
                ...
                direction: cotrader_core::TradeDirection::Short,
    ...
    /// Generate a trade signal reusing the prior debate_layer result.
    /// Avoids running a redundant second debate inside strategy_decision.
    pub async fn generate_signal_with_debate(
        &self,
        symbol: &str,
        current_price: f64,
    ...
    pub async fn generate_signal_with_aggregation(
        ...
        symbol: &str,
        current_price: f64,
        aggregated_signal: Option<&cotrader_core::AggregatedSignal>,
        ...
        let patterns_for_levels: Vec<cotrader_core::CandlestickPattern> = {
        ...
        if let Some(ref det) = det_strategy_result {
            ...
            primary_action = if det.direction == cotrader_core::TradeDirection::Long {
                "BUY"
            } else {
                "SELL"
            };
        ...
        } else if let Some(verdict) = prior_debate {
            // Reuse the debate_layer result — avoids running a second debate
            (verdict.action.clone(), verdict.confidence, verdict.reasoning.clone(), vec![])
        } else {
        ...
        // Vector memory context — semantic search for similar past regimes
        let vector_context = {
        ...
        if skip_debate_si_llm {
            ...
            let max_per_symbol = max_from_equity.min(max_from_cash) * 0.98;
            ...
            if sigma > 0.030 {
                ...
                signal.stop_loss = match signal.direction {
                    cotrader_core::TradeDirection::Long => signal.entry_price - risk_dist,
                    cotrader_core::TradeDirection::Short => signal.entry_price + risk_dist,
                };
                // FIXED (R:R audit): the stop just widened 1.30× — the dollar risk per
                // share grew 1.30×, so shares must shrink 1.30× to keep risk at 1% of
                // capital (MATH). Previously size was computed from the ORIGINAL stop,
                // silently risking 1.3% per trade in high-vol regimes.
                signal.position_size /= 1.30;
                // FIXED (R:R audit): TP contraction removed — shrinking TP 15% while
                // widening SL 30% degraded 2:1 setups to 1.31:1 (observed live).
                // The R:R floor below re-extends TP to ≥2R after the wider stop.
                if risk_dist > 0.0 {
                    let reward_dist = (signal.take_profit - signal.entry_price).abs();
                    signal.risk_reward_ratio = reward_dist / risk_dist;
                }
            }

            // ═══ R:R DISCIPLINE — 1% capital risk must chase ≥2% capital reward ═══
            let (tp_enforced, rr_enforced) = crate::helpers::enforce_min_risk_reward(
                signal.direction,
                signal.entry_price,
                signal.stop_loss,
                signal.take_profit,
            ...
            if (tp_enforced - signal.take_profit).abs() > f64::EPSILON {
                println!(
                    "[StrategyDecision] 📐 R:R below 2:1 — extending TP {:.4} → {:.4}",
                    signal.take_profit, tp_enforced
                );
            }
            signal.take_profit = tp_enforced;
            signal.risk_reward_ratio = rr_enforced;

            println!(
                "[StrategyDecisionAgent] 🚀 FAST-PATH: {} {} @ entry={:.2} SL={:.2} TP={:.2} (RR {:.1}:1, conf {:.1}%, sigma={:.4})",
                if signal.direction == cotrader_core::TradeDirection::Long { "BUY" } else { "SELL" },
        ...
        if let Some(ref agg_signal) = aggregated_signal {
            ...
            let (_trading_mode, _daily_goal_context) = {
            ...
            // LLM decision (opinion only) — cross-check the deterministic signal
            let llm_request = LLMRequest {
                request_id: format!("strategy-{}-{}", symbol, Utc::now().timestamp_micros()),
                agent_role: cotrader_core::role::AgentRole::StrategyDecision,
                prompt: format!(
                    "You are a trading signal generator. Respond with exactly one word: BUY, SELL, or HOLD.\n\nSymbol: {}\nPrice: {:.2}\nConfluence: {:.0}%\nTrend: {}\nPivot: {:.2}\nRSI: {:.1}\nHeat: {:.1}%\n\nSignal:",
                    symbol, current_price, reliance * 100.0, trend_label, pivots.pivot, rsi, portfolio_heat * 100.0
                ),
            ...
            let llm_decision: Option<LlmTradeDecision> = {
                ...
                // LLM is "available" if it responded with a valid action.
                // HOLD is a valid trading decision — not an error.
                let available = !decision.reason.contains("Parse failed")
                    && !decision.reason.contains("unavailable")
                    && !decision.reason.contains("timed out")
                ...
                if available {
                    self.state
                        ...
                        .await;
                    // Track LLM decision in communication log with chain-of-reasoning
                    let llm_verdict = match decision.action.as_str() {
                        "BUY" => DecisionVerdict::Buy {
                            reason: decision.reason.clone(),
                            confidence: 0.6,
                        },
                        "SELL" => DecisionVerdict::Sell {
                            reason: decision.reason.clone(),
                            confidence: 0.6,
                        },
                        _ => DecisionVerdict::Hold {
                            reason: decision.reason.clone(),
                        },
                    };
                    self.state
                        .push_agent_decision(
                            AgentDecision::new("LLM", symbol, llm_verdict)
                                .with_evidence(vec![
                                    ...
                                    format!("si_says: {}", si_final_action),
                                    format!("cross_check_only: true"),
                                ])
                    ...
                    self.state
                        ...
                        .await;
                    // Track LLM HOLD/unavailable in communication log
                    self.state
                        .push_agent_decision(
                            AgentDecision::new(
                                "LLM",
        ...
        // ═══ R:R DISCIPLINE — 1% capital risk must chase ≥2% capital reward ═══
        let (signal_tp, final_rr) = crate::helpers::enforce_min_risk_reward(
            direction, signal_entry, signal_sl, signal_tp, 2.0,
        );
        ...
        println!(
            ...
            if direction == cotrader_core::TradeDirection::Long { "BUY" } else { "SELL" },
