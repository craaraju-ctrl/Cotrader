...
impl OnChainData {
    ...
    async fn fetch_coingecko_onchain(
        ...
        // Retry up to 2 times on failure (CoinGecko rate limits aggressively)
        ...
        if v == serde_json::Value::Null {
            return Err(format!("CoinGecko failed after 3 attempts: {:?}", last_err).into());
        }
    ...
    fn calculate_volume_signal(&self, bars: &[cotrader_core::OhlcvBar]) -> f64 {
        if bars.len() < 20 {
            return 0.5;
        }
    ...
    fn calculate_price_signal(&self, bars: &[cotrader_core::OhlcvBar]) -> f64 {
        if bars.len() < 10 {
            return 0.5;
        }
    ...
    fn calculate_volatility_signal(&self, bars: &[cotrader_core::OhlcvBar]) -> f64 {
        if bars.len() < 20 {
            return 0.5;
        }
