...
pub struct DebateContext {
    ...
    pub aggregated_signal: Option<cotrader_core::AggregatedSignal>,
...
impl DebateLayer {
    ...
    async fn assemble_context(
        ...
        let (vector_memory_matches, skill_votes_bullish, skill_votes_bearish, skill_votes_neutral) = {
            ...
            let vm_count = if vm.is_empty() { 0 } else { 3 };
            let votes = self.state.agent_memory.last_skill_votes.read().await;
            let (mut bull, mut bear, mut neut) = (0usize, 0usize, 0usize);
            for v in votes.iter() {
                match v.direction {
                    cotrader_core::agent::SkillDirection::Bullish => bull += 1,
                    cotrader_core::agent::SkillDirection::Bearish => bear += 1,
                    _ => neut += 1,
                }
            }
    ...
    async fn build_signal_from_verdict(
        ...
        let pivots = cotrader_core::calculate_pivot_points(
            ctx.price * 1.01,
            ctx.price * 0.99,
            ctx.price * 0.998,
            rules.pivot_method,
        ...
        // Use 0.5% minimum stop distance to prevent degenerate levels for penny assets.
        let min_stop_dist = entry * 0.01;
        let stop_dist = (entry - stop_loss).abs();
        let levels_valid = match direction {
            cotrader_core::TradeDirection::Long => {
                stop_loss < entry && take_profit > entry && stop_dist >= min_stop_dist * 0.5
            }
            cotrader_core::TradeDirection::Short => {
                stop_loss > entry && take_profit < entry && stop_dist >= min_stop_dist * 0.5
            }
        ...
        if !levels_valid {
            stop_loss = if direction == cotrader_core::TradeDirection::Long {
                entry - min_stop_dist
            } else {
                entry + min_stop_dist
            };
            take_profit = if direction == cotrader_core::TradeDirection::Long {
