    ...
    async fn execute(
        ...
        if let AgentInput::ConfluenceRequest { context } = input {
            ...
            let (direction, score) = if snap.depth_score > 0.7 {
                ...
                (cotrader_core::agent::SkillDirection::Bullish, snap.depth_score)
            } else if snap.depth_score < 0.3 {
                // Poor liquidity = slippage risk = Bearish
                (cotrader_core::agent::SkillDirection::Bearish, snap.depth_score)
            } else {
                // Adequate liquidity = Neutral
                (cotrader_core::agent::SkillDirection::Neutral, snap.depth_score)
            };
