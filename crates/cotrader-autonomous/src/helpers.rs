...
use cotrader_core::{DisciplineRules, OhlcvBar};
...
/// R:R discipline — the math/physics contract of the system:
///   MATH:    risk is fixed at 1R (`max_risk_per_trade` of equity, enforced by sizing),
///            so reward must be ≥ `min_rr` × risk (2R ⇒ 1% risk chases 2% of capital).
///   PHYSICS: volatility modulation may widen the stop (noise protection); when it
///            does, the take-profit MUST be re-extended, otherwise the geometry decays
///            (observed live: SL×1.30 + TP×0.85 turned 2:1 setups into 1.31:1).
///
/// If the strategy's TP is closer than `min_rr` × stop distance, extend it.
/// Returns the corrected `(take_profit, risk_reward_ratio)`.
pub fn enforce_min_risk_reward(
    direction: cotrader_core::TradeDirection,
    entry_price: f64,
    stop_loss: f64,
    take_profit: f64,
    min_rr: f64,
) -> (f64, f64) {
    let risk_dist = (entry_price - stop_loss).abs();
    if risk_dist <= 0.0 || entry_price <= 0.0 {
        return (take_profit, 0.0);
    }
    let reward_dist = match direction {
        cotrader_core::TradeDirection::Long => take_profit - entry_price,
        cotrader_core::TradeDirection::Short => entry_price - take_profit,
    };
    let rr = reward_dist / risk_dist;
    if rr >= min_rr {
        return (take_profit, rr);
    }
    let extended = match direction {
        cotrader_core::TradeDirection::Long => entry_price + risk_dist * min_rr,
        cotrader_core::TradeDirection::Short => entry_price - risk_dist * min_rr,
    };
    (extended, min_rr)
}
...
pub fn calculate_position_size_with_cash(
    ...
    // FIX: Reject if stop distance is negligible (< 0.5% of entry price).
    // This prevents position size explosion for penny assets (e.g., ADA at $0.19
    // with SL at $0.1887 would produce 100,000+ shares without this guard).
    if stop_distance < entry_price * 0.005 {
        return 0.0;
    }
...
pub fn calculate_risk_reward(
    ...
    match direction {
        cotrader_core::TradeDirection::Long if target > entry => reward / risk,
        cotrader_core::TradeDirection::Short if target < entry => reward / risk,
        _ => 0.0,
    }
}
...
pub fn compute_autonomous_levels(
    _symbol: &str,
    current_price: f64,
    pivots: &cotrader_core::PivotLevels,
    _patterns: &[cotrader_core::CandlestickPattern], // patterns come from MI state; kept for future extension
    ...
    forced_direction: Option<cotrader_core::TradeDirection>,
) -> (f64, f64, f64, f64) {
    // entry, sl, tp, rr
    let mut direction = forced_direction.unwrap_or(cotrader_core::TradeDirection::Long);
    ...
    if let Some(agg) = aggregated_signal {
        if let Some(forced) = forced_direction {
            ...
            if (forced == cotrader_core::TradeDirection::Long && agg.is_bullish(None))
                || (forced == cotrader_core::TradeDirection::Short && agg.is_bearish(None))
            {
    ...
    let entry = if direction == cotrader_core::TradeDirection::Long {
    ...
    let sl = if direction == cotrader_core::TradeDirection::Long {
    ...
    let tp = if direction == cotrader_core::TradeDirection::Long {
    ...
    let final_tp = if rr < min_rr {
        if direction == cotrader_core::TradeDirection::Long {
            entry + risk * min_rr
        } else {
            entry - risk * min_rr
        }
