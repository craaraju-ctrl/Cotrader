...
impl PortfolioManagerAgent {
    ...
    pub async fn update_position_pnl(
        ...
        if updated {
            ...
            // FIXED (rules audit): drawdown must see UNREALIZED losses too.
            // Previously max_drawdown_today only updated on position CLOSE from
            // realized daily_pnl — observed live: 9 open losing positions with
            // drawdown_pct=0.0, so the 2% daily hard limit could never fire
            // intraday. Floating P&L = realized today + open unrealized.
            ...
            // Halt on breach — same policy as the close path, but now it can
            // trigger while positions are still open (when it matters most).
            let rules = self.state.rule_engine.rules.read().await;
            if portfolio.trading_enabled
                && portfolio.max_drawdown_today >= rules.max_daily_drawdown
            {
                portfolio.trading_enabled = false;
                println!(
                    "[PortfolioManager] 🛑 TRADING HALTED — floating drawdown {:.2}% ≥ {:.2}% daily limit",
    ...
    pub async fn add_position(
        ...
        // ═══ CAPITAL ALLOCATION RULE (Fix #3: cash-aware, mode-gated) ══
        // Live trading: each symbol can consume at most 4% (1/25) of total
        // equity — the hard discipline rule. Paper mode: relaxed to 95% so
        // paper runs get enough trade frequency to validate strategy logic
        // without real capital at risk (see strategy_decision.rs for the
        // matching pattern used at signal-sizing time).
        // The effective cap must also respect available cash — using only
        // total_equity * pct is wrong when cash is already deployed.
        ...
        let max_from_equity = total_equity * equity_pct;
        ...
        if position_value > max_per_symbol {
            return Err(format!(
                "Position value ₹{:.2} exceeds cap ₹{:.2} (equity={:.2}, cash={:.2}, mode={})",
                position_value,
                max_per_symbol,
                total_equity,
                cash_available,
...
mod tests {
    ...
    async fn test_short_position_accounting() {
        ...
        let state = SharedState::new(memory, rules, config, "test_portfolio_history.db", paper_broker)
