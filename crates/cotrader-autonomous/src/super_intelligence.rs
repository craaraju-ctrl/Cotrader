...
use cotrader_core::agent::SkillDirection;
use cotrader_core::paper_engine::TradingMode;
use cotrader_core::AggregatedSignal;
use cotrader_core::ConfirmationLevel;
...
impl SuperIntelligence {
    ...
    pub async fn analyze(
        ...
        // based on the full conviction stack + cross-validation + ML scoring
        ...
        // ML signal quality scoring — predicts P(profitable) from features
        let ml_signal_features: Vec<f64> = {
            let mut features = Vec::with_capacity(34);
            // 8 conviction factors
            features.push(conviction.directional_conviction);
            features.push(conviction.confidence_conviction);
            features.push(conviction.agreement_conviction);
            features.push(conviction.memory_conviction);
            features.push(conviction.risk_conviction);
            features.push(conviction.pattern_conviction);
            features.push(conviction.timeframe_alignment);
            features.push(conviction.synthesis_score);
            // 26 indicator features (placeholder — real impl pulls from MetricsSnapshot)
            features.extend_from_slice(&vec![0.5; 26]);
            features
        };
        let (ml_profitability, ml_source) = state.ml_engine.score_signal(
            &ml_signal_features, conviction.final_conviction,
        ).await;

        // Blend ML score with conviction: ML acts as a cross-check
        // If ML disagrees strongly (P(profitable) < 0.3), penalize conviction
        let ml_adjustment = if ml_source == "ml" {
            if ml_profitability < 0.3 {
                -0.15 // ML says unlikely to profit — reduce conviction
            } else if ml_profitability > 0.7 {
                0.05 // ML agrees — slight boost
            } else {
                0.0 // ML is uncertain — no adjustment
            }
        } else {
            0.0 // No ML model — no adjustment
        };

        let adjusted_conviction = (conviction.final_conviction + ml_adjustment).clamp(0.0, 1.0);

        let should_buy = adjusted_conviction >= regime_threshold
            && validation.overall_validation_score >= min_validation;

        let recommended_action = if proposed_action == "BUY" && should_buy {
            "BUY"
        } else if proposed_action == "HOLD" && should_buy && adjusted_conviction >= 0.60 {
        ...
        let recommended_confidence = if recommended_action == "BUY" {
            // Conviction-stacked confidence with ML adjustment
            (adjusted_conviction * 0.6 + validation.overall_validation_score * 0.2 + ml_profitability * 0.2)
                .clamp(0.0, 0.95)
        } else {
        ...
        println!(
            "[SuperIntelligence] {} → {} (conf {:.1}%) | conviction={:.1}% ml_adj={:+.2} ml_profit={:.1}%({}) validation={:.1}% memory={}ep | {}",
            proposed_action,
            recommended_action,
            recommended_confidence * 100.0,
            adjusted_conviction * 100.0,
            ml_adjustment * 100.0,
            ml_profitability * 100.0,
            ml_source,
