...
fn render_rules_panel(f: &mut Frame, area: Rect, app: &AppState) {
    ...
    lines.push(Line::from(vec![
        ...
        conf_badge,
        Span::styled(
            "  [toggle: rat rules use_confluence=true]",
    ...
    lines.push(Line::from(vec![
        ...
        sess_badge,
        Span::styled(
            "  [toggle: rat rules respect_session_timing=true]",
            Style::default().fg(Color::Rgb(60, 60, 80)),
        ),
    ]));
    lines.push(Line::from(""));
    ...
    lines.push(Line::from(""));
    lines.push(Line::from(vec![Span::styled(
        "  ℹ️  Set rules via: rat rules key=value",
...
fn render_recent_comm(f: &mut Frame, area: Rect, app: &AppState) {
    ...
    for (from, _to, msg, ts) in &app.live_comm_log {
        ...
        let from_col = if from.contains("Rat") || from.contains("Meta") {
            Color::Cyan
