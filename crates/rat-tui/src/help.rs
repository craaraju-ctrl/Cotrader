...
pub fn render_help(f: &mut Frame, area: Rect) {
    let help = r"
rat Terminal UI — Trading Real-time Edge Decision Optimisation

This is the primary way to observe and lightly control the autonomous system.

The real brain (temporal loops + Rat agents + debate + skills) runs in `rat-orchestrator`.

Key commands (from any terminal):
  rat                 # start everything (or launch this UI)
  rat tui             # explicitly launch this full Terminal UI
  rat health / status / cot / portfolio / tree
  rat watchlist add TSLA
  rat rules min_confluence_score=0.72
