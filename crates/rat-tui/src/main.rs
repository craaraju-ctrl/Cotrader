//! rat-tui — The full beautiful Terminal UI for
//! Trading Real-time Edge Decision Optimisation (rat).
//!
//! This is the PRIMARY interface. The autonomous brain (rat-orchestrator)
...
fn api_base() -> String {
    let port: u16 = std::env::var("PORT")
        ...
        .or_else(|| {
            if let Ok(content) = std::fs::read_to_string("config/rat.env") {
    ...
    if !io::stdout().is_terminal() {
        anyhow::bail!(
            "TUI requires an interactive terminal.\n\
             Start headless: rat start --headless\n\
             Attach TUI later from a real terminal: rat tui"
        );
    }
    enable_raw_mode()?;
        ...
        "cot" => {
            ...
            let to_agent = if let Some(to) = json.get("to").and_then(|t| t.as_str()) {
                ...
                match agent_name.as_str() {
                    n if n.contains("Rat") || n.contains("Meta") => "Identifier".to_string(),
