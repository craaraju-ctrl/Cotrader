...
Thank you for your interest in contributing to rat — Trading Real-time Edge Decision Optimisation.
...
```bash
git clone https://github.com/craaraju-ctrl/Rat.git
cd Rat

# Copy environment template
cp config/rat.env.example config/rat.env
...
Copy `config/rat.env.example` to `config/rat.env` and edit as needed. Source it before running:

```bash
source config/rat.env
```
...
rat is a **Rust-first** workspace organised into these crates:

| Crate | Purpose |
|-------|---------|
| `rat-core` | Foundation: DisciplinedCore rules, memory (redb), LLM client, AgentSkill trait, paper engine, BrokerAdapter trait, backtest engine |
| `rat-autonomous` | Agent hierarchy, debate, skills implementations, reflection, meta-control, state, pipeline, new: DebateOrchestrator, OutcomeProcessor, RegimeClassifier |
| `rat-orchestrator` | Temporal loops (fast/medium/slow), HTTP/WS API server |
| `rat-tui` | Primary Terminal UI (ratatui) — COT tree, portfolio, agent views, policy cache, backtest, health, performance |
| `rat-server` | Production HTTP server (Axum + broker registry + paper/live mode switching) |
| `rat-runtime` | **Unified runtime engine** — event-driven, multi-mode (paper/live/backtest/validate/research), world model, policy cache, active learner, broker plugin system, introspector |
| `rat-broker-alpaca` | Alpaca Markets API v2 broker adapter (US equities + crypto, paper + live) |
| `rat-broker-zerodha` | Zerodha Kite Connect v3 broker adapter (India equities + derivatives) |
...
- **Skills** — implement the `AgentSkill` trait from `rat-core/src/skills.rs`
- **Debate roles** — extend the debate system in `rat-autonomous/src/debate.rs`
- **Runtime modules** — add to `rat-runtime/src/` with proper event bus integration
...
- Use `PolicyCache` (in `rat-runtime`) for learned trading memory
...
in the [Actions tab](https://github.com/craaraju-ctrl/Rat/actions).
...
| Tauri | Checks `rat-tauri` crate compiles with GTK deps |
| Kronos | Validates Python service syntax |
| Broker adapters | Tests `rat-broker-alpaca` and `rat-broker-zerodha` in paper mode |
...
```bash
# Core tests
cargo test -p rat-core

# Agent tests
cargo test -p rat-autonomous

# Runtime tests
cargo test -p rat-runtime

# Broker adapter tests (paper mode)
cargo test -p rat-broker-alpaca -p rat-broker-zerodha

# Integration tests (13 tests covering all agent groups)
cargo test -p rat-autonomous --test rat_integration
```
...
cargo run -p rat-runtime -- --mode validate --cycles 50

# Or via legacy launcher
source config/rat.env
PORT=8082 cargo run -p rat-orchestrator &
cargo run -p rat-tui   # Open TUI to observe live
...
- Open a [GitHub Discussion](https://github.com/craaraju-ctrl/Rat/discussions)
