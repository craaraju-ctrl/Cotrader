...
impl KnowledgeGraph {
    ...
    pub fn detect_communities(
        ...
        // Use the store's public API to get all node IDs
        let node_ids = self.store.get_all_graph_node_ids()?;

        // Now use public API to fetch edges
        let mut edges: Vec<(String, String)> = Vec::new();
        for node_id in &node_ids {
            let node_edges = self.get_edges(node_id)?;
            for e in &node_edges {
                let pair = (e.source_id.clone(), e.target_id.clone());
    ...
    pub fn get_hubs(&self, limit: usize) -> rusqlite::Result<Vec<(String, usize)>> {
        ...
        for node_id in &node_ids {
            ...
            results.push((node_id.clone(), deg));
        }

        results.sort_by_key(|&(_, deg)| std::cmp::Reverse(deg));
    ...
    pub fn remove_edges(&self, record_id: &str) -> rusqlite::Result<u64> {
        self.store.remove_edges_for(record_id)
    }
