...
pub struct OpenAICompatState {
    ...
    /// Reusable HTTP client (avoids creating a new reqwest::Client per request)
...
pub async fn chat_completions(
    ...
    let search_results = match state.client.get(&search_url).send().await {
...
pub async fn health_check(State(state): State<OpenAICompatState>) -> impl IntoResponse {
    let health = match state.client.get(format!("{}/health", state.api_url)).send().await {
        Ok(resp) if resp.status().is_success() => resp
