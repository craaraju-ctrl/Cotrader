...
impl EvolutionEngine {
    ...
    /// Create an evolution engine with custom temporal decay configuration.
    pub fn with_decay_config(store: MemoryStore, config: EvolutionConfig, decay_config: DecayConfig) -> Self {
        let consolidation = ConsolidationEngine::new(store.clone());
        let reflection = ReflectionEngine::new(store.clone());
        let temporal = TemporalEngine::new(store.clone(), decay_config);
        Self {
            store,
            consolidation,
            reflection,
    ...
    pub fn prune_stale(&self) -> rusqlite::Result<Vec<EvolutionEvent>> {
        ...
        for tier in MemoryTier::all() {
            ...
            for candidate in &candidates {
                ...
                // Build a TemporalFact for decay calculation
                let fact = candidate.to_temporal_fact();
                ...
                if effective_importance < 0.2 && candidate.access_count < 3 {
                    ...
                    if tier == MemoryTier::Semantic || tier == MemoryTier::Procedural {
                        let archived_id = format!("archived_{}", crate::generate_id());
                        let mut archived_record = candidate.record.clone();
                        archived_record.id = archived_id;
                        archived_record.metadata.insert("archived".into(), "true".into());
                        archived_record
                            .metadata
                            .insert("archived_at".into(), Utc::now().to_rfc3339());
                        archived_record
                            .metadata
                            .insert("original_id".into(), id.clone());
                        let _ = self.store.insert(&archived_record);
                    }
    ...
    /// Run a reflexion loop: reflect on current state, assess, plan improvements.
    /// This is the core self-improvement mechanism from MemGPT/Letta.
    pub fn run_reflexion(&self, topic: &str, observation: &str) -> (Reflection, SelfAssessment) {
        self.reflection.reflexion_loop(topic, observation)
    }
    ...
    /// Run a full sleep-time cycle: consolidate + tune + prune + distill + reflexion.
    ...
    pub fn run_sleep_cycle(&mut self) -> SleepCycleReport {
        ...
        let mut report = SleepCycleReport {
            cycle_id: format!("sleep_{}", crate::generate_id()),
        ...
        // Phase 3: Stale pruning (now includes temporal decay-aware pruning)
        ...
        // Phase 5: Reflexion — reflect on the sleep cycle results
        let total_events = report.tuning_events.len()
            + report.pruning_events.len()
            + report.distillation_events.len();
        let obs = format!(
            "Sleep cycle complete. Consolidated, tuned, pruned, and distilled {} total events. \
             {} records evicted, {} promoted, {} deduplicated.",
            total_events,
            report.consolidation_report.as_ref().map_or(0, |c| c.records_evicted),
            report.consolidation_report.as_ref().map_or(0, |c| c.records_promoted),
            report.consolidation_report.as_ref().map_or(0, |c| c.records_deduplicated),
        );
        let (reflection, assessment) = self.reflection.reflexion_loop("sleep_cycle", &obs);
        report.reflection = Some(reflection);
...
pub struct SleepCycleReport {
    ...
    /// Reflexion from this sleep cycle
    pub reflection: Option<Reflection>,
    /// Self-assessment from this sleep cycle
