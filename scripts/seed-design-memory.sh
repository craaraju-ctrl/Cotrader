...
# Seeds the Rat Exchange Design System into agentmemory.
...
if [ ! -f "$SKILL_FILE" ]; then
  ...
  echo "   Run from the Rat project root."
  exit 1
fi

echo "🧠 Seeding Rat Exchange Design System into agentmemory..."
echo "   API: $BASE_URL"
echo ""
...
CODE=$(post_record \
  "design-color-brand" \
  "Rat Exchange Design System — Brand & Colors:
- Rat Yellow (primary): #FCD535 — single brand color for CTAs, headlines, wordmark
- Rat Yellow Active (pressed): #f0b90b
- Rat Yellow Disabled: #3a3a1f
...
CODE=$(post_record \
  ...
  "Rat Exchange Design System — Trading Semantics:
...
CODE=$(post_record \
  "design-typography" \
  "Rat Exchange Design System — Typography:
...
CODE=$(post_record \
  "design-layout" \
  "Rat Exchange Design System — Layout:
...
CODE=$(post_record \
  "design-components" \
  "Rat Exchange Design System — Components:
...
CODE=$(post_record \
  "design-trading-desk" \
  "Rat Exchange Design System — Trading Desk:
...
echo "   → Search: tantra search 'rat design'"
echo "   → Recall: AgentMemoryClient.recall('rat design system colors')"
