...
**Lightweight FastAPI microservice** exposing the Kronos foundation model as a REST API for the rat trading system.

Kronos supplies short-horizon time-series forecasts that enrich market context and strategy decisions. The service is optional — rat operates with graceful degradation when it is unavailable.
...
```mermaid
graph TB
    subgraph "rat Rust Backend"
        ORCH[rat Orchestrator]
...
The Rust side (`rat-core/src/kronos_client.rs`) calls this service over HTTP.
...
It also feeds the skills layer (via MI) and debate participants (Proposer/Critic etc. incorporate regime/volatility signals that benefit from accurate short-term forecasts). Part of the overall **Strong Skills + Rules + Trained Memory** intelligence system (see root README and rat-core/src/skills.rs).
...
| **Timeout** | rat client has a 5-second timeout; service typically responds in 100-500ms |
| **Graceful Degradation** | If Kronos is unavailable, rat falls back to Neutral trend direction |
