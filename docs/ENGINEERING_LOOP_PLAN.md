# RAT — Engineering Loop Completion Plan
...
(`crates/rat-orchestrator/src/loops.rs`) — knowledge-graph rebuild → deep
...
`loop_cadence_secs()` (`RAT_FAST_LOOP_SECS` / `RAT_MEDIUM_LOOP_SECS` /
`RAT_SLOW_LOOP_SECS`), defaults unchanged. Documented in `config/rat.env.example`.

**Verify:** `cargo build -p rat-orchestrator`, then run with
`RAT_SLOW_LOOP_SECS=120` and watch for `Meta-review completed` / `RULE_ADAPT` /
...
- Added CLI subcommand `rat-cli self-evolve [cycles] [--induce] [--symbols ...]`
...
path on an env flag (e.g. `RAT_INDUCE_REGRET_SL_PCT`) that the validator sets,
...
**Re-audit (2026-06-27):** already implemented in `crates/rat-core/src/paper_engine.rs`:
...
- `crates/rat-core/src/vector_memory.rs` currently brute-forces cosine over a
...
- **Verify:** `cargo build -p rat-core --features lancedb`.

## 5. Richer skills + meta-adaptation of skills — PARTIAL
- `crates/rat-core/src/agent.rs`: add a structured `SkillResult { score, note }`
