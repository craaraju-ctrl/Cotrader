...
```mermaid
gantt
    title rat Development Timeline
...
| Milestone | Status | Description |
|-----------|--------|-------------|
| Clean Rust repository | ✅ Done | Workspace with `rat-core`, `rat-autonomous`, `rat-orchestrator` crates |
...
| Milestone | Status | Description |
|-----------|--------|-------------|
| Unified Runtime Engine | ✅ Done | `rat-runtime` with event-driven architecture, multi-mode support |
...
| Alpaca Broker Adapter | ✅ Done | `rat-broker-alpaca` — US equities + crypto, paper + live |
| Zerodha Broker Adapter | ✅ Done | `rat-broker-zerodha` — India equities + derivatives, OAuth2 |
...
| Skills Layer (AgentSkill trait) | ✅ Done | `rat-core/src/skills.rs`: trait + TrainedMemorySkill + impls for SentimentAnalyzer, VolatilityCalculator, NewsAnalyser, MarketMetricsMeter |
