...
## 🏗️ Agent Hierarchy & Logical Groups (Rat)

rat structures its agents into the **`Rat`** four-group orchestrator (Identifier / Verifier / Executer / Guardian). The beautiful Terminal UI (`rat tui`) is the primary way to watch and control the live system.

The **Runtime Engine** (`rat-runtime`) wraps this hierarchy into an event-driven, multi-mode system with a world model, policy cache, and broker plugin system.

```mermaid
graph TB
    subgraph "Rat Logical Hierarchy"
    ...
    subgraph "Runtime Layer (rat-runtime)"
        RT[RuntimeEngine]
        WM[WorldModelEngine]
        PCACHE[PolicyCache]
        AL[ActiveLearner]
...
Agents and sub-agents **already know what to do** (their Rat role and responsibilities in the five-layer pipeline).
...
- **Skills** (`rat-core/src/skills.rs` — the `AgentSkill` trait + `TrainedMemorySkill` + `SkillWrapper`) tell them **how to do** things. Pluggable and executable: SentimentAnalyzer, VolatilityCalculator, regime detection, patterns, and the key `TrainedMemorySkill`. Agents/sub-agents can hold or receive `Vec<Box<dyn AgentSkill>>` and call `execute`.
- **Rules** (`rat-core/src/disciplined_core.rs`) tell **what to do and what not to do** (`DisciplineRules`, `validate_trade_setup`, `check_risk_limits`, etc.). These are now memory-aware via `apply_trained_memory_to_rules` (past regret/lessons can tighten confluence or risk limits dynamically).
...
- **Policy Cache** (`rat-runtime/src/policy_cache.rs`) is the **learned trading memory** that records (market features → action → outcome) tuples and short-circuits expensive Ollama debate when the system has seen a similar setup before.
- **World Model** (`rat-runtime/src/world_model.rs`) maintains persistent beliefs about symbols, cross-symbol correlations, macro state, and active hypotheses.
